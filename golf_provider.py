...
def _player_row(c):
    ...
    # ESPN sometimes omits athlete.id on the leaderboard; if so, every player
    # would collapse to "" and the UI would open the same person for every click.
    # Fall back to the competitor id, then a stable slug of the player's name so
    # each row has a unique, consistent id (board, matchup, and projection all
    # resolve through this same id).
    _nm = ath.get("displayName") or ath.get("fullName") or ""
    _pid = ath.get("id") or c.get("id")
    if not _pid:
        _slug = re.sub(r"[^a-z0-9]+", "-", _nm.lower()).strip("-")
        _pid = ("p-" + _slug) if _slug else ""
...
def get_board(tour=DEFAULT_TOUR, dates=None):
    from collections import Counter
    tour = tour if tour in TOURS else DEFAULT_TOUR
    url = f"{BASE}/{tour}/scoreboard"
    if dates:
        url += f"?dates={dates}"           # YYYYMMDD or YYYYMMDD-YYYYMMDD (finished events)
    data = _get(url, _TTL_LIVE)
