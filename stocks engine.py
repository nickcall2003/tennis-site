...
WATCHLIST = [t.strip().upper() for t in os.environ.get(
    "STOCKS_WATCHLIST",
    "AAPL,MSFT,NVDA,AMZN,GOOGL,META,TSLA,NFLX,DIS,JPM,V,COST,AMD,SNAP,SBUX"
).split(",") if t.strip()]
BENCHMARK = "SPY"
...
STOCKS = [t.strip().upper() for t in os.environ.get(
    "STOCKS_STOCKS", "AAPL,MSFT,NVDA,AMZN,GOOGL,META,TSLA,NFLX,DIS,AMD,SNAP,SBUX,JPM,COST,GPRO"
).split(",") if t.strip()]
ETFS = [t.strip().upper() for t in os.environ.get(
    "STOCKS_ETFS", "SPY,QQQ,VTI,DIA,IWM,ARKK,XLK,XLF,GLD"
).split(",") if t.strip()]
CRYPTO = [t.strip().upper() for t in os.environ.get(
    "STOCKS_CRYPTO", "BTC-USD,ETH-USD,SOL-USD,XRP-USD,DOGE-USD,ADA-USD"
).split(",") if t.strip()]
