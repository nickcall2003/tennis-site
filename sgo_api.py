...
def _league_events(league):
    ...
    league per CACHE_TTL, reused for every game in that window."""
...
def get_player_props(sport, home, away):
    ...
    de-vigged over probability, median over/under price). Returns [] on:
    no key, unsupported sport, no event match, or no props posted yet."""
    if not API_KEY or sport not in SGO_LEAGUE:
        return []
    events = _league_events(SGO_LEAGUE[sport])
    if not events:
        return []
