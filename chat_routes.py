...
- PLAYER PROPS: use the lookup_props tool whenever the user asks about props, projections, or which props to bet, ladder, or parlay. It returns the model's real projection, the book line, the diff (edge), the odds, and the model's context (opponent defense + pace).
...
def _lookup_props(team_or_matchup, stat=None):
    ...
    try:
        ...
        for p in (data.get("props") or []):
            if stat and (stat or "").lower() not in (str(p.get("label", "")) + str(p.get("stat", ""))).lower():
                continue
            proj, line = p.get("projection"), p.get("line")
            if proj is None or line is None:
                continue
            rows.append({
                "player": p.get("player"), "team": p.get("team"), "stat": p.get("label"),
                "line": line, "model_projection": proj,
                "diff": round(float(proj) - float(line), 1),
                "lean": "OVER" if float(proj) > float(line) else "UNDER",
                "over_odds": p.get("over_odds"), "under_odds": p.get("under_odds"),
                "model_context": p.get("context"),        # real opp-defense/pace factors
            })
        rows.sort(key=lambda x: -abs(x["diff"]))
        return {"found": True, "sport": sport, "game": tname, "props": rows[:14],
                "note": ("model_projection is the model's number; diff = projection - line. "
                         "Only these figures are real \u2014 do not add outside stats.")}
    except Exception as e:
        return {"found": False, "error": str(e)[:150]}
