...
def _build(ev, league):
    ...
    ch = h["score"] or 0
    ca = a["score"] or 0
    ph, pd, pa = _winprob(h["win_pct"], a["win_pct"], ch, ca, minute, live)
    top = max(ph, pd, pa)
    ...
    # ESPN moneyline odds (3-way) when present, for market price + edge
    odds = None
    od = comp.get("odds") or []
    if od:
        o = od[0] or {}
        ...
        ho = _ml(o.get("homeTeamOdds"))
        ao = _ml(o.get("awayTeamOdds"))
        do = _ml(o.get("drawOdds"))
        ...
        if do is None and isinstance(o.get("drawMoneyLine"), (int, float)):
            do = o["drawMoneyLine"]
        if ho is not None or ao is not None:
            odds = {"ml_home": ho, "ml_away": ao, "ml_draw": do}
    eh, ea = _exp_goals(h["win_pct"], a["win_pct"])
    return {
