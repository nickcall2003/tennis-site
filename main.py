...
def model_lookup(team: str, sport: str | None = None, days: int = 3):
    ...
    # bots send things like "Nationals ML" / "Yankees moneyline" — strip the
    # market suffix so the team name still matches
    for suf in (" ml", " moneyline", " money line", " spread", " total",
                " over", " under", " rl", " run line", " puck line"):
    ...
    for offset in range(days):
        ...
        for p in plays:
            if sport and p.get("sport") != sport:
                continue
            # search every field that can carry a team name, not just match/pick.
            # the raw play often holds abbreviations (WSH @ ATH) while the
            # rendered pick line holds full names (Washington Nationals).
            bits = [p.get("match"), p.get("pick"), p.get("home"), p.get("away"),
                    p.get("home_name"), p.get("away_name"), p.get("team"),
                    p.get("player"), p.get("home_team"), p.get("away_team")]
            try:
    ...
    return {"found": False, "team": team, "searched_for": needle,
            "plays_scanned": len(scanned), "samples": scanned[:8],
