...
# ── FAST BOARD FOR THE DISCORD BOT ────────────────────────────────────────────
# /api/picks/best runs 2 live LLM calls PER PLAY (narrate.prose for reason +
# premium), which makes it 30-60s+ on a full slate. The Discord bot's /today and
# /prop only need matchup/pick/prob/odds/edge — NOT the LLM prose. This endpoint
# returns the same board with odds enrichment but SKIPS all LLM generation, so it
# responds in ~1-2s. Safe to delete; nothing else depends on it.
...
def picks_quick(date: str | None = None, sport: str | None = None,
    ...
    for p in plays:
        ...
        if (p.get("prob") or 0) < min_prob:
            continue
        p = dict(p)  # don't mutate the cached play
        try:
            _enrich_odds(p)  # market_odds + fair_odds + edge_pct, no LLM
        except Exception:
            pass
        if min_edge > 0 and (p.get("edge_pct") is None or p.get("edge_pct", 0) < min_edge):
            continue
    ...
    # bot sorts by edge itself; return the whole quick board
    return {"date": target.isoformat(), "count": len(out), "picks": out}
