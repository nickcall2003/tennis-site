...
async def lifespan(app: FastAPI):
    ...
    if USE_REAL and run_bg and startup_build:
        ...
        _thr.Thread(target=_warm_quick_board, daemon=True).start()
...
def _warm_quick_board():
    """Keep the enriched board cache hot.

    Building the board calls the odds providers per play (tennis hits its API
    once per match), so a cold rebuild can take long enough to time out the
    caller — which is what made the daily Discord card miss. This refreshes the
    cache in the background a little before it expires, so every real request
    (site, bot, daily post) hits a warm copy. PICKS_QUICK_WARM=0 disables."""
    import time as _t
    if os.environ.get("PICKS_QUICK_WARM", "1") != "1":
        print("[warm] quick-board warmer disabled")
        return
    ttl = int(os.environ.get("PICKS_QUICK_TTL", "600") or 600)
    interval = max(60, int(ttl * 0.8))
    _t.sleep(90)   # let the app finish coming up first
    while True:
        try:
            t0 = _t.time()
            res = picks_quick()
            print(f"[warm] board cached: {res.get('count', 0)} plays "
                  f"in {_t.time() - t0:.1f}s")
        except Exception as e:
            print(f"[warm] quick board failed: {e}")
        _t.sleep(interval)
...
def tennis_sofa_debug(date: str | None = None):
    ...
    try:
        ...
        # Broad path discovery: try many candidate URL shapes at once and report
        # each HTTP status, so ONE request reveals the route that returns 200.
        try:
            out["path_discovery"] = provider.discover_schedule_path(dk)
        except Exception as e:
            out["path_discovery_error"] = str(e)
...
# --- capper endpoints live in capper_routes.py (extracted to keep main.py small) ---
...
# --- golf endpoints live in golf_routes.py (extracted to keep main.py small).
# Wrapped like the other routers: if the file is missing from a deploy, golf
# 404s but the site boots. ---
try:
    from golf_routes import router as _golf_router
    app.include_router(_golf_router)
except Exception as _e:
    print(f"[startup] golf router not loaded: {_e}")
...
@app.get("/api/model")
def _match_sides(match: str):
    """Split a board matchup into its two sides. Team sports are '{away} @ {home}',
    tennis is '{player_a} vs {player_b}'. Returns [] if the format is unknown —
    callers must treat that as 'side unresolved', never as a default."""
...
def _norm_name(s):
    """Lowercase, accent-free, punctuation-free form for name comparison."""
    import unicodedata as _ud
...
def _name_matches(needle, name):
    """Does `needle` name `name`? Handles the abbreviated forms the tennis and
    UFC feeds use: the board stores 'F. C. Jianu' while people type 'Filip Jianu'
    or just 'Jianu', and a plain substring test misses both. Surname must match;
    if a forename was given, its initial must agree, so 'J. Smith' won't answer
    to 'Mike Smith'."""
    n, s = _norm_name(needle), _norm_name(name)
    if not n or not s:
        return False
    if n in s:
        return True
    nt, st = n.split(), s.split()
    if not nt or not st or nt[-1] != st[-1]:
        return False
    return len(nt) == 1 or nt[0][0] == st[0][0]


def _ufc_lookup(target, team):
    """UFC isn't part of _gather_plays, so /track could never find a fighter.
    Read the card directly and return a play-shaped dict."""
    try:
        import ufc_provider
        games = ufc_provider.get_games(target) or []
    except Exception as e:
        print(f"[model_lookup] ufc fetch failed for {target}: {e}")
        return None
    try:
        _attach_odds("ufc", games)
    except Exception:
    ...
    for g in games:
        ...
        if not home or not away:
            continue
        matched = next((s for s in (away, home) if _name_matches(team, s)), None)
        if not matched:
            continue
        ph = g.get("prob_home")
        pa = g.get("prob_away")
        if pa is None and ph is not None:
            pa = 1.0 - float(ph)
        ph, pa = float(ph or 0), float(pa or 0)
        pick = home if ph >= pa else away
        prob = max(ph, pa)
        o = g.get("odds") or {}
        mo = o.get("ml_home") if pick == home else o.get("ml_away")
        fair = (-round(100 * prob / (1 - prob)) if prob >= 0.5 and prob < 1
                else round(100 * (1 - prob) / prob) if 0 < prob < 0.5 else None)
        return {
            "found": True, "date": target.isoformat(), "sport": "ufc",
            "ref": str(g.get("id")) if g.get("id") is not None else None,
            "match": f"{away} vs {home}", "pick": pick, "prob": prob,
            "win_pct": round(prob * 100), "confidence": g.get("confidence"),
            "market_odds": mo, "fair_odds": fair,
            "edge_pct": None, "has_edge": False,
            "event_time": g.get("event_time") or g.get("time"),
            "sides": [away, home], "matched_side": matched,
            "side_is_model_pick": matched == pick,
        }
    return None
...
def model_lookup(team: str, sport: str | None = None, days: int = 3):
    ...
    IMPORTANT: `pick` is the MODEL's side. It is NOT necessarily the side the
    caller searched for — searching 'Marlins' finds the Marlins/Astros game even
    when the model likes the Astros. `matched_side` is the side the search term
    actually names, and is what a caller recording a USER's bet must use. Reading
    `pick` for that purpose silently books people onto the opposite team."""
    ...
    for offset in range(days):
        ...
        for p in plays:
            ...
            # which side did the caller actually name? name-aware first, so
            # abbreviated tennis entries still resolve, then plain substring.
            sides = _match_sides(p.get("match"))
            matched = next((s for s in sides if _name_matches(team, s)), None)
            if matched is None and needle not in hay:
                continue
            p = dict(p)
            try:
                _enrich_odds(p)
            ...
            edge = p.get("edge_pct")
            pick = p.get("pick")
            is_model_pick = bool(matched and pick and
                                 _norm_name(matched) == _norm_name(pick))
            return {
                "found": True, "date": target.isoformat(), "sport": p.get("sport"),
                "ref": str(p.get("id")) if p.get("id") is not None else None,
                "match": p.get("match"), "pick": pick, "prob": prob,
                ...
                "sides": sides, "matched_side": matched,
                "side_is_model_pick": is_model_pick,
            }
        # UFC lives outside _gather_plays entirely — search the card separately.
        # get_games() already falls forward to the next card up to 60 days out, so
        # one call covers the whole lookahead; repeating it per day would refetch
        # the same bouts.
        if offset == 0 and (not sport or sport == "ufc"):
            hit = _ufc_lookup(target, team)
            if hit:
                return hit
...
# --- capper endpoints (see capper_routes.py). Registered last so every helper
# the routes reach back for (model_lookup, _catchup_grade_cappers) exists.
# Wrapped so a missing file can't crash the boot. ---
try:
