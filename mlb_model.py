...
class TeamInput:
    ...
    starter_metric: str = "ERA"             # what starter_era ACTUALLY holds:
                                            # the provider feeds FIP when it has
                                            # it (fielding-independent, better
                                            # predictor) and falls back to ERA.
                                            # The driver breakdown must say which
                                            # or the printed number won't match
                                            # the ERA a reader looks up.
...
def confidence_notes(home: TeamInput, away: TeamInput):
    """WHY the confidence is what it is, in plain language.

    A bare "medium" tells a reader nothing and reads like hedging. Naming the
    actual gap — an unannounced starter, a 14-inning sample — is more useful
    AND more credible than a number presented as if every game were equally
    knowable. Returns [] when there's nothing to flag."""
    notes = []
    for t, side in ((home, "Home"), (away, "Away")):
        nm = t.name or side
        if not t.runs_per_game:
            notes.append(f"No season offensive data for {nm} — using league average.")
        if not t.starter_era:
            notes.append(f"{nm} starter not announced — the projection leans on "
                         f"bullpen and team form instead.")
        elif t.starter_ip is not None and t.starter_ip < 20:
            notes.append(f"{nm} starter has only {t.starter_ip:.0f} innings this "
                         f"season — small sample, treat the number as soft.")
        if not t.bullpen_era:
            notes.append(f"No bullpen data for {nm} — league average assumed.")
        if t.bullpen_fatigue and t.bullpen_fatigue > 0.6:
            notes.append(f"{nm} bullpen is heavily used lately — run prevention "
                         f"is downgraded, but relief workload is noisy.")
    return notes
...
def _driver_chain(off_rpg, opp_starter_era, opp_bullpen_era, gf, opp_fatigue,
                  side_mult, side_label, opp_metric="ERA"):
    ...
    steps = [
        ...
        {"key": "opp_pitching", "label": "Opponent pitching", "mult": prevent,
         "detail": _pitch_detail(opp_starter_era, opp_bullpen_era, opp_fatigue, gf.lg_era, opp_metric)},
...
def _pitch_detail(s_era, b_era, fatigue, lg_era, metric="ERA"):
    bits = []
    if s_era:
        bits.append(f"starter {s_era:.2f} {metric}")
    if b_era:
...
def predict_game(home: TeamInput, away: TeamInput, gf: GameFactors | None = None):
    ...
    home_steps, home_chain = _driver_chain(
        ...
        away.bullpen_fatigue, HOME_RUN_BUMP, "Home field", away.starter_metric)
    away_steps, away_chain = _driver_chain(
        away.blended_rpg(), home.starter_era, home.bullpen_era, gf,
        home.bullpen_fatigue, AWAY_RUN_DAMP, "Road split", home.starter_metric)
    ...
    return {
        ...
        "model_notes": confidence_notes(home, away),
        "drivers": {
