    ...
    for r in rows:
        ...
        for k, v in r["stats"].items():
            try:
                ...
                continue
            if "game" in k.lower():
                totals[k] = totals.get(k, 0.0) + x        # games DO sum
                weights[k] = 1.0
            else:
                totals[k] = totals.get(k, 0.0) + x * g
                weights[k] = weights.get(k, 0.0) + g
    career = {}
    for k, tot in totals.items():
        w = weights.get(k) or 1.0
        val = tot if w == 1.0 else tot / w
        career[k] = int(val) if abs(val - round(val)) < 1e-9 else round(val, 1)
