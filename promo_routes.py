...
def build_ladder(day=None, force=False):
    ...
    try:
        ...
        # never let the ladder hang or vanish — return a valid "holding" shape
        data = {"date": day.isoformat(), "leg": None, "error": str(e)[:120],
...
def _build_ladder_inner(day=None):
    ...
    # filter on REAL market price (the band) and the model's edge. Covers EVERY
    # sport (tennis via api-tennis, team sports via snapshots), not just MLB.
    try:
        import main as _m
        for p in (src or []):
