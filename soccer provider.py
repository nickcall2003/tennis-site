...
# ---- Head-to-head season series (soccer has draws -> W-D-L) ------------------
_TEAM_SCHED: dict = {}     # (slug, team_id) -> [{"opp":id,"res":"w"/"d"/"l"}, ...]
_H2H_BUDGET = [0]          # cap NEW schedule fetches per build so a board can't stall


def _team_schedule(slug, team_id):
    key = (slug, str(team_id))
    ...
    out = []
    try:
        base = _SCOREBOARD.format(slug=slug).rsplit("/scoreboard", 1)[0]
        data = E._get(f"{base}/teams/{team_id}/schedule", {})
        for ev in data.get("events", []):
            ...
            if not (((comp.get("status") or {}).get("type") or {}).get("completed")):
                continue
            res = "w" if me.get("winner") is True else ("l" if opp.get("winner") is True else "d")
            out.append({"opp": str((opp.get("team") or {}).get("id")), "res": res})
    except Exception:
        out = []
    _TEAM_SCHED[key] = out
    return out
...
def _season_h2h(league, home_id, away_id):
    if not home_id or not away_id:
        return None
    try:
        slug = _SLUG.get(league, league)
        games = [g for g in _team_schedule(slug, home_id) if g["opp"] == str(away_id)]
        if not games:
            return None
        w = sum(1 for x in games if x["res"] == "w")
        d = sum(1 for x in games if x["res"] == "d")
        l = sum(1 for x in games if x["res"] == "l")
        return {"w": w, "d": d, "l": l, "record": f"{w}-{d}-{l}",
                "games": len(games), "seasons": 1, "draws": True}
    except Exception:
        return None
...
def team_profile(team_id, name=None, league=None):
    """Honest soccer team profile (W-D-L record, splits, form, goals for/against)
    from the team's real ESPN schedule for the given competition."""
    out = {"sport": "soccer", "team_id": str(team_id), "name": name,
           "score_term": "Goals", "draws": True, "league": league}
    slug = _SLUG.get(league, league) if league else None
    if not slug:
        return out
    try:
        base = _SCOREBOARD.format(slug=slug).rsplit("/scoreboard", 1)[0]
    ...
    for ev in data.get("events", []):
        ...
        res = "w" if me.get("winner") is True else ("l" if opp.get("winner") is True else "d")
        games.append({"res": res, "home": me.get("homeAway") == "home",
    ...
    out = TP.build(out, games, draws=True)
    _attach_apifootball(out, out.get("name") or name, league)
    return out


# our soccer league key -> api-football league id (majors api-football covers)
_AF_LEAGUE = {"epl": 39, "ucl": 2, "laliga": 140, "mls": 253, "seriea": 135,
              "bundesliga": 78, "ligue1": 61, "uel": 3, "uecl": 848, "worldcup": 1,
              "ligamx": 262, "championship": 40, "eredivisie": 88, "ligaportugal": 94,
              "saudi": 307}


def _af_league_season(league):
    lid = _AF_LEAGUE.get(league)
    if not lid:
        return None, None
    import datetime as dt
    t = dt.date.today()
    # api-football season = the year the season STARTED. European leagues run
    # Aug->May; MLS/Liga MX/Saudi are closer to calendar-year, so bias to current.
    if league in ("mls", "ligamx", "saudi"):
        season = t.year
    else:
...
def _attach_apifootball(out, team_name, league):
    """Best-effort: add api-football's richer season stats (goal averages, clean
    sheets, failed-to-score, biggest win/loss, formations) to a soccer profile.
    A miss (no key, off-season, unmatched) just leaves the profile as-is."""
    try:
        import apifootball
        if not apifootball.available() or not team_name:
            return
        lid, season = _af_league_season(league)
        if not lid:
            return
        st = apifootball.team_stats(team_name, lid, season)
        if not st or st.get("unmatched"):
            return
        out["adv"] = {
            "source": "api-football", "season": st.get("season"),
            "record": st.get("record"), "gf_avg": st.get("gf_avg"),
            "ga_avg": st.get("ga_avg"), "clean_sheets": st.get("clean_sheets"),
            "failed_to_score": st.get("failed_to_score"),
            "biggest_win": st.get("biggest_win"), "biggest_loss": st.get("biggest_loss"),
            "formations": st.get("formations"),
        }
    except Exception:
        pass
...
def _build(ev, league, ext=None):
    ...
    return {
        ...
        "h2h": _season_h2h(league, home_id, away_id),
