...
_raw_pre_cache = {}                                     # dg_tour -> (ts, raw json)
...
def _fetch_pre(dgt):
    """One pre-tournament fetch per tour (returns both models); cached raw."""
    c = _raw_pre_cache.get(dgt)
    ...
    _raw_pre_cache[dgt] = (time.time(), data)
    return data


def pre_tournament(tour="pga", model="baseline"):
    """Returns:
      {"event","updated","model","players":{norm:{name,dg_id,win,top5,top10,top20,make_cut}}}
    or None. model is 'baseline' (skill at a neutral course) or 'fit'
    (baseline + course history & fit). Cached per (tour, model)."""
    if not _KEY:
        return None
    dgt = TOUR_MAP.get((tour or "pga").lower(), "pga")
    want_fit = model in ("fit", "history_fit", "baseline_history_fit")
    ck = f"pre:{dgt}:{'fit' if want_fit else 'baseline'}"
    c = _cache.get(ck)
    if c and time.time() - c[0] < _TTL:
        return c[1]
    data = _fetch_pre(dgt)
    ...
    if isinstance(data, dict):
        if want_fit:
            rows = data.get("baseline_history_fit") or data.get("baseline") or []
        else:
    ...
    out = {"event": (data.get("event_name") if isinstance(data, dict) else None),
           ...
           "model": ("fit" if want_fit else "baseline"), "players": players}
...
def matchups(tour="pga", market="3_balls"):
    ...
    Cached ~60s so live polling doesn't hammer the DataGolf API."""
    if not _KEY:
        return None
    dgt = TOUR_MAP.get((tour or "pga").lower(), "pga")
    ck = f"mu:{dgt}:{market}"
...
def matchup_board(tour="pga"):
    ...
    for mkey, mlabel, slots in spec:
        ...
        for m in ml:
            ...
            # order by model preference and flag the model's favorite to win
            players.sort(key=lambda p: (p["model_pct"] if p["model_pct"] is not None
                                        else -1), reverse=True)
            players[0]["fav"] = True
