...
def _read_local_csv(path):
    """Read a Sackmann match CSV already on disk (e.g. from a cloned repo)."""
    try:
...
def aggregate_from_dirs(dirs, start, end, no_wta, store):
    """Aggregate from local cloned repos. Looks for atp_matches_YYYY.csv and
    wta_matches_YYYY.csv (tour-level singles) within [start, end]."""
    import glob
    total = 0
        ...
        for pre in prefixes:
            for path in sorted(glob.glob(os.path.join(d, pre + "*.csv"))):
                ...
                if not (yr.isdigit() and start <= int(yr) <= end):
                    continue   # skip doubles/qual/itf/amateur and out-of-range
...
def main():
    ...
    ap.add_argument("--from-dir", action="append", default=[], metavar="DIR",
                    help="read CSVs from a local cloned repo instead of the network "
                         "(repeatable, e.g. --from-dir tennis_atp --from-dir tennis_wta)")
    args = ap.parse_args()
