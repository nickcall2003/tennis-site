...
ATP_URL = "https://raw.githubusercontent.com/JeffSackmann/tennis_atp/refs/heads/master/atp_matches_{year}.csv"
WTA_URL = "https://raw.githubusercontent.com/JeffSackmann/tennis_wta/refs/heads/master/wta_matches_{year}.csv"
# Fallback to the legacy bare-branch form if the refs/heads form ever misses.
ATP_URL_ALT = "https://raw.githubusercontent.com/JeffSackmann/tennis_atp/master/atp_matches_{year}.csv"
WTA_URL_ALT = "https://raw.githubusercontent.com/JeffSackmann/tennis_wta/master/wta_matches_{year}.csv"
...
def _fetch_csv(url: str):
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "linelogic-surface/1.0"})
        with urllib.request.urlopen(req, timeout=60) as r:
            text = r.read().decode("utf-8", "replace")
        return list(csv.DictReader(io.StringIO(text)))
    except urllib.error.HTTPError as e:
        # Some networks 404 the bare-branch path; retry the other URL form once.
        if e.code == 404 and "/refs/heads/master/" in url:
            return _fetch_csv(url.replace("/refs/heads/master/", "/master/"))
        if e.code == 404 and "/refs/heads/master/" not in url and "/master/" in url:
            return _fetch_csv(url.replace("/master/", "/refs/heads/master/"))
        print(f"[surf]   skip {url.rsplit('/', 1)[-1]}: {e}")
        return []
