...
Env:
  ...
  DISCORD_MENTION=@everyone   ping used on alerts
"""
import os
...
def run_and_alert(post=True):
    """Daily job: compute signals, update the paper book, alert new buys/sells."""
    import stocks_engine as E
    signals, prices = _compute_all()
    buys, sells = E.run_day(signals, prices)
    result = {"buys": buys, "sells": sells}
    if post and (buys or sells):
...
def stocks_search(q: str = ""):
    """Look up any symbol by ticker or company name."""
...
def stocks_portfolio():
    import stocks_engine as E
    try:
        _, prices = _compute_all()
    except Exception:
        prices = None
    return E.snapshot(prices)
