...
# ---- Sweep-avoidance motivation (a small, tracked situational edge) ----------
# A team facing elimination-by-sweep tends to lift; a team trying to COMPLETE a
# sweep on the ROAD tends to underperform (the desperate home side plays up). We
# nudge the sweep-AVOIDER's win probability by a small, tunable amount, with a
# little extra when the sweep-seeker is on the road. It's deliberately modest and
# attached to the game object so it can be verified and proven/killed by CLV.
_SWEEP_BASE = float(os.environ.get("SWEEP_MOTIVATION", "0.025"))   # base boost to the avoider
_SWEEP_ROAD = float(os.environ.get("SWEEP_ROAD_BONUS", "0.015"))   # extra when seeker is on the road
_SERIES_WIN: dict = {}   # date_key -> [{"h":id,"a":id,"w":winner_id}, ...] recent finals


def _series_window(key):
    """Completed games over the ~6 days ending at `key`, used to derive the current
    series score. One cached schedule call; any failure degrades to no data (and
    therefore no adjustment)."""
    if key in _SERIES_WIN:
        return _SERIES_WIN[key]
    out = []
    try:
        end = dt.date.fromisoformat(key)
        start = end - dt.timedelta(days=6)
        sched = _get(f"{BASE}/schedule",
                     {"sportId": 1, "startDate": start.isoformat(),
                      "endDate": end.isoformat(), "gameType": "R", "hydrate": "linescore"})
        for d in sched.get("dates", []):
            for g in d.get("games", []):
                ...
                if hs is None or as_ is None:
                    continue
                out.append({"h": ht["team"]["id"], "a": at["team"]["id"],
                            "w": (ht["team"]["id"] if hs > as_ else at["team"]["id"])})
    except Exception:
        out = []
    _SERIES_WIN[key] = out
    return out
...
_SEASON_GAMES: dict = {}   # team_id -> [{"opp":id,"won":bool}, ...] this season's finals


def _team_season_games(team_id):
    """This season's completed games for a team (cached). Lets us derive the
    head-to-head record vs any opponent without a per-matchup call."""
    if team_id in _SEASON_GAMES:
        return _SEASON_GAMES[team_id]
    out = []
    try:
        sched = _get(f"{BASE}/schedule",
        ...
        for d in sched.get("dates", []):
            for g in d.get("games", []):
                if (g.get("status", {}) or {}).get("abstractGameState") != "Final":
                    continue
                ht = g["teams"]["home"]; at = g["teams"]["away"]
                hid, aid = ht["team"]["id"], at["team"]["id"]
                hs, as_ = ht.get("score"), at.get("score")
                if hs is None or as_ is None:
                    continue
                if team_id == hid:
                    out.append({"opp": aid, "won": hs > as_})
                elif team_id == aid:
                    out.append({"opp": hid, "won": as_ > hs})
    except Exception:
        out = []
    _SEASON_GAMES[team_id] = out
    return out


def _season_h2h(home_id, away_id):
    """Home team's season record vs the away team, e.g. {'record':'7-8'}."""
    games = [x for x in _team_season_games(home_id) if x["opp"] == away_id]
    if not games:
        return None
    w = sum(1 for x in games if x["won"])
    return {"w": w, "l": len(games) - w, "record": f"{w}-{len(games)-w}", "games": len(games)}


def team_profile(team_id, name=None):
    """Honest MLB team profile (record, splits, form, streak, runs for/against)
    from the team's real season schedule."""
    out = {"sport": "mlb", "team_id": str(team_id), "name": name, "score_term": "Runs"}
    try:
        team_id = int(team_id)
    except Exception:
        pass
    ...
    for d in sched.get("dates", []):
        for g in d.get("games", []):
            ...
            if hs is None or as_ is None:
                continue
            if team_id == hid:
                home, opp_t, ms, os_ = True, at["team"], hs, as_
            elif team_id == aid:
                home, opp_t, ms, os_ = False, ht["team"], as_, hs
            else:
                continue
            if not out.get("name"):
                out["name"] = (ht["team"] if home else at["team"]).get("name")
            games.append({"won": ms > os_, "home": home,
                          "opp": opp_t.get("abbreviation") or opp_t.get("name"),
                          "ms": ms, "os": os_, "date": (g.get("gameDate", "") or "")[:10]})
    import team_profile_util as TP
    return TP.build(out, games)


def _sweep_context(window, home_id, away_id, game_num, games_in_series):
    """Sweep setup for TODAY's game, or None. Only the final game of a series can
    be a sweep clincher, and only if one side won every prior game in it."""
    try:
        gn, gs = int(game_num or 0), int(games_in_series or 0)
    except Exception:
        return None
    if gs < 2 or gn != gs:                       # must be the series' final game
        return None
    prior = [x for x in window if {x["h"], x["a"]} == {home_id, away_id}][-(gn - 1):]
    if len(prior) < gn - 1:                       # can't confirm the series score
        return None
    hw = sum(1 for x in prior if x["w"] == home_id)
    aw = sum(1 for x in prior if x["w"] == away_id)
    if aw == 0 and hw == gn - 1:                  # home swept so far -> away faces the sweep
        return {"facing": "away", "seeker": "home", "seeker_road": False, "lead": hw}
    if hw == 0 and aw == gn - 1:                  # away swept so far (road) -> home faces it
        return {"facing": "home", "seeker": "away", "seeker_road": True, "lead": aw}
    return None
...
def get_games(date: dt.date, force_live=False):
    ...
    for g in raw_games:
        ...
        home = TeamInput(name=home_t["team"].get("name", "Home"), abbr=hmeta.get("abbr", ""),
                         ...
                         starter_era=(hp.get("fip") if hp.get("fip") is not None else hp["era"]),
                         starter_metric=("FIP" if hp.get("fip") is not None else "ERA"),
                         starter_ip=hp["ip"], bullpen_era=hs["bullpen_era"],
        ...
        away = TeamInput(name=away_t["team"].get("name", "Away"), abbr=ameta.get("abbr", ""),
                         ...
                         starter_era=(ap.get("fip") if ap.get("fip") is not None else ap["era"]),
                         starter_metric=("FIP" if ap.get("fip") is not None else "ERA"),
                         starter_ip=ap["ip"], bullpen_era=as_["bullpen_era"],
        ...
        # --- sweep-avoidance motivation: nudge the team facing elimination-by-sweep
        series_note = None
        try:
            if status != "finished" and (_SWEEP_BASE or _SWEEP_ROAD):
                sc = _sweep_context(_series_window(key), home_id, away_id,
                                    g.get("seriesGameNumber"), g.get("gamesInSeries"))
                if sc:
                    boost = _SWEEP_BASE + (_SWEEP_ROAD if sc["seeker_road"] else 0.0)
                    p = pred["prob_home"]
                    if sc["facing"] == "home":
                        p = min(0.95, p + boost)          # boost home (the avoider)
                    else:
                        p = max(0.05, p - boost)          # boost away (lower home prob)
                    pred["prob_home"] = p
                    series_note = {"game": g.get("seriesGameNumber"), "of": g.get("gamesInSeries"),
                                   "facing_sweep": sc["facing"], "sweep_seeker": sc["seeker"],
                                   "road_sweep": sc["seeker_road"], "adj": round(boost, 3)}
        except Exception:
            series_note = None

        # Calibrate stated confidence to the realized hit rate (learned from
        # pick_results). Shrinks toward 0.5, so it never changes which side we pick
        # — only how confident we claim to be — and tempers inflated "edges".
        try:
            import mlb_calibration
            pred["prob_home"] = mlb_calibration.calibrate(pred["prob_home"])
        except Exception:
            pass
        ...
        games.append({
            ...
            "series": series_note,
            "h2h": _season_h2h(home_id, away_id),
            "exp_runs_home": pred["exp_runs_home"], "exp_runs_away": pred["exp_runs_away"],
            "factors": pred["factors"], "drivers": pred.get("drivers"),
            "model_notes": pred.get("model_notes") or [],
...
def _side(t: TeamInput, raw, pitcher):
    ...
    return {
        ...
        "starter": {
            "name": t.starter_name,
            # the model runs on FIP (far more predictive of the next start), but
            # "ERA" must mean ERA — showing FIP under an ERA label made our
            # numbers look wrong next to ESPN's
            "era": (pitcher or {}).get("era"),
            "fip": (pitcher or {}).get("fip"),
            "model_input": t.starter_era,
            "ip": t.starter_ip,
        },
    }
...
_team_split_cache = {}   # (team_id, hand) -> (ts, {k_pct, avg, ops, whiff-ish})


def _pitcher_hand(pid):
    """'L' or 'R' \u2014 the starter's throwing hand (drives platoon splits)."""
    if not pid:
        return None
    try:
        data = _get(f"{BASE}/people/{pid}")
        return ((data["people"][0].get("pitchHand") or {}).get("code") or "").upper()[:1] or None
    except Exception:
        return None


def _pitcher_name(pid):
    """Full name for a pitcher id, or None. Used to look the starter up in
    Statcast (which keys on name, not MLBAM id)."""
    if not pid:
        return None
    try:
        data = _get(f"{BASE}/people/{pid}")
        return (data["people"][0].get("fullName") or "").strip() or None
    except Exception:
        return None


def _team_split_vs_hand(team_id, hand):
    """The opponent lineup's REAL season batting split vs LHP or RHP, from the
    official MLB Stats API (sitCodes vl/vr). Gives strikeout rate, AVG and OPS \u2014
    the honest version of 'this lineup whiffs against lefties'."""
    if not team_id or hand not in ("L", "R"):
        return None
    key = (str(team_id), hand)
    c = _team_split_cache.get(key)
    if c and time.time() - c[0] < _DAY_TTL:
        return c[1]
    out = None
    try:
        ...
        out = None
    _team_split_cache[key] = (time.time(), out)
    return out


# League-average K% vs a given hand (used as the neutral baseline for the boost).
_LEAGUE_K_PCT = float(os.environ.get("MLB_LEAGUE_K_PCT", "22.4"))
# League-average batting AVG (neutral baseline for the platoon contact factor).
_LEAGUE_AVG = float(os.environ.get("MLB_LEAGUE_AVG", "0.243"))
...
def get_props(date: dt.date, game_id: int):
    ...
    for side, pid, opp in (("home", home_pid, g["away"]["name"]),
        ...
        if not rate:
            continue
        # ADVANCED: adjust the K projection by how much the OPPOSING lineup actually
        # strikes out against this pitcher's handedness (official MLB split data).
        opp_side = "away" if side == "home" else "home"
        hand = _pitcher_hand(pid)
        split = _team_split_vs_hand(g[opp_side].get("team_id"), hand)
        adj_rate, ctx = rate, None
        if split and split.get("k_pct"):
            f = split["k_pct"] / _LEAGUE_K_PCT if _LEAGUE_K_PCT else 1.0
            f = max(0.85, min(1.18, f))          # bounded so one split can't run away
            adj_rate = rate * f
            hl = "LHP" if hand == "L" else "RHP"
            ctx = (f"{opp} strike out {split['k_pct']}% of PA vs {hl} "
                   f"(league {_LEAGUE_K_PCT}%) \u2014 {'+' if f >= 1 else ''}{round((f-1)*100)}% to the K projection")
        # STATCAST LAYER: the pitcher's best swing-and-miss pitch vs how this
        # lineup actually whiffs against THAT pitch type. Real numbers only; if
        # Savant is unavailable or the sample is thin, this simply doesn't apply.
        sc_ctx = None
        try:
            import statcast as SC
            prof = SC.pitcher_profile(name)
            bw = (prof or {}).get("best_whiff_pitch")
            if bw and bw.get("pitch"):
                batters, _ = _lineup_or_roster(game_id, g[opp_side].get("team_id"), opp_side)
                names = [b.get("name") for b in (batters or [])[:9] if b.get("name")]
                lw = SC.lineup_whiff_vs_pitch(names, bw["pitch"])
                if lw and lw.get("league_whiff_pct"):
                    wf = lw["whiff_pct"] / lw["league_whiff_pct"]
                    wf = max(0.92, min(1.10, wf))   # smaller bound: layered on top
                    adj_rate *= wf                  # of the handedness K% factor
                    pn = bw["pitch"]
                    sc_ctx = (f"His best whiff pitch is the {pn} ({bw['whiff_pct']}% whiff); "
                              f"this lineup whiffs {lw['whiff_pct']}% vs {pn} "
                              f"(league {lw['league_whiff_pct']}%) \u2014 "
                              f"{'+' if wf >= 1 else ''}{round((wf-1)*100)}%")
                    if prof.get("arm_angle") is not None:
                        sc_ctx += f" \u00b7 {round(prof['arm_angle'])}\u00b0 arm slot"
                    fb = prof.get("fastball") or {}
                    fbits = []
                    if fb.get("velo"):
                        fbits.append(f"{round(fb['velo'],1)} mph")
                    if fb.get("spin"):
                        fbits.append(f"{round(fb['spin'])} rpm")
                    if fb.get("break_z_vs_avg") is not None:
                        _r = fb["break_z_vs_avg"]
                        fbits.append(f"{'+' if _r >= 0 else ''}{round(_r,1)}\" ride vs avg")
                    if fbits:
                        sc_ctx += " \u00b7 FB " + ", ".join(fbits)
        except Exception:
            sc_ctx = None
        line = default_line("strikeouts", adj_rate)
        proj = project_prop("strikeouts", adj_rate, line)
        if proj:
            proj["player"] = name
            proj["team"] = g[side]["name"]
            proj["opponent"] = opp
            proj["label"] = "Strikeouts"
            proj["season_k_avg"] = round(rate, 1)
            parts = [c for c in (ctx, sc_ctx) if c]
            # plain-English reason for the lean, built only from factors we used
            try:
                _why = []
                if rate:
                    _why.append(f"He averages {round(rate,1)} K per start.")
                if split and split.get("k_pct"):
                    _hi = split["k_pct"] >= _LEAGUE_K_PCT
                    _why.append(f"{opp} {'strike out more' if _hi else 'make more contact'} "
                                f"than average ({split['k_pct']}% vs {_LEAGUE_K_PCT}% league).")
                if sc_ctx and "whiffs" in sc_ctx:
                    _why.append("His best swing-and-miss pitch is factored in.")
                if _why:
                    proj["why"] = " ".join(_why)
            except Exception:
                pass
            if parts:
                proj["context"] = " \u00b7 ".join(parts)
                if split:
                    proj["opp_split"] = split
            if sc_ctx:
                proj["statcast"] = True
            out.append(proj)
...
def get_batter_props(date: dt.date, game_id: int, max_batters=9):
    ...
    # ADVANCED: each lineup faces the OPPOSING starter. Adjust batter projections by
    # that lineup's real season split vs. the starter's handedness (official MLB
    # split data) — the honest "this lineup hits lefties well/poorly" factor.
    starters = {"home": _starter_id(date, game_id, "home"),
                "away": _starter_id(date, game_id, "away")}
    out = []
    for side_key in ("home", "away"):
        team_id = g[side_key]["team_id"]
        opp_key = "away" if side_key == "home" else "home"
        opp_hand = _pitcher_hand(starters.get(opp_key))
        split = _team_split_vs_hand(team_id, opp_hand)
        # AVG vs that hand relative to league average AVG -> bounded contact factor
        hit_f, ctx = 1.0, None
        try:
            if split and split.get("avg"):
                avg_vs = float(split["avg"])
                lg = _LEAGUE_AVG
                if lg:
                    hit_f = max(0.88, min(1.14, avg_vs / lg))
                    hl = "LHP" if opp_hand == "L" else "RHP"
                    ctx = (f"lineup hits {split['avg']} vs {hl} (league {lg:.3f}) \u2014 "
                           f"{'+' if hit_f >= 1 else ''}{round((hit_f-1)*100)}% to contact props")
        except (TypeError, ValueError):
            hit_f, ctx = 1.0, None
        batters, _ = _lineup_or_roster(game_id, team_id, side_key)
        # STATCAST: what pitch will these hitters actually see most from the
        # opposing starter, and how does each hitter really fare against it?
        _sc_prof = None
        try:
            import statcast as SC
            _opp_starter = starters.get(opp_key)
            if _opp_starter:
                _nm = _pitcher_name(_opp_starter)
                _sc_prof = SC.pitcher_profile(_nm) if _nm else None
        except Exception:
            _sc_prof = None
        _prim = ((_sc_prof or {}).get("primary_pitch") or {}).get("pitch")
        for b in batters[:max_batters]:
            rates = _batter_rates(b["id"])
            if not rates:
                continue
            # this hitter vs that specific pitch (real sample required)
            bvp, bvp_f = None, 1.0
            if _prim:
                try:
                    import statcast as SC
                    bvp = SC.batter_vs_pitch(rates["name"], _prim)
                    if bvp and bvp.get("ba") is not None and bvp.get("league_ba"):
                        bvp_f = max(0.90, min(1.12, bvp["ba"] / bvp["league_ba"]))
                except Exception:
                    bvp, bvp_f = None, 1.0
            for stat in ("hits", "home_runs", "rbis"):
                ...
                if rate <= 0:
                    continue
                # contact factor applies to hits/RBIs; HR is power-driven, so use a
                # softened version rather than pretending AVG predicts homers.
                f = hit_f if stat in ("hits", "rbis") else (1 + (hit_f - 1) * 0.5)
                # layer the hitter's real performance vs the pitch he'll see most;
                # power props get half weight (batting average predicts contact,
                # not power)
                bf = bvp_f if stat in ("hits", "rbis") else (1 + (bvp_f - 1) * 0.5)
                adj = rate * f * bf
                line = default_line(stat, adj)
                proj = project_prop(stat, adj, line)
                if not proj:
                    continue
                proj["player"] = rates["name"]
                proj["team"] = g[side_key]["name"]
                proj["label"] = labels[stat]
                proj["season_avg"] = round(rate, 2)
                bits = [c for c in (ctx,) if c]
                if bvp and bvp.get("league_ba"):
                    _d = round((bvp_f - 1) * 100)
                    bits.append(f"vs {_prim}: hits {bvp['ba']:.3f} "
                                f"(league {bvp['league_ba']:.3f}, {bvp['pa']:.0f} PA) "
                                f"\u2014 {'+' if _d >= 0 else ''}{_d}%")
                if bits:
...
# ===================== PLAYER PROFILES =====================
# Full season + career lines, situational splits (RISP, vs LHP/RHP), and the
# Statcast pitch-type breakdown. All from the official MLB Stats API (free, no
# key) plus Baseball Savant. Any piece that fails is simply omitted — a profile
# never shows a number we didn't actually retrieve.

_profile_cache = {}
_PROFILE_TTL = 1800


def _stat_group(pid, stats, group, season_=None, sit=None):
    """One /people/{id}/stats call -> the first split's stat dict, or None."""
    params = {"stats": stats, "group": group}
    if season_:
        params["season"] = season_
    if sit:
        params["sitCodes"] = sit
    try:
...
_HIT_KEYS = ["gamesPlayed", "atBats", "runs", "hits", "doubles", "triples",
             "homeRuns", "rbi", "baseOnBalls", "strikeOuts", "stolenBases",
             "avg", "obp", "slg", "ops", "babip", "plateAppearances"]
_PIT_KEYS = ["gamesPlayed", "gamesStarted", "wins", "losses", "era", "whip",
             "inningsPitched", "strikeOuts", "baseOnBalls", "hits", "homeRuns",
             "saves", "strikeoutsPer9Inn", "walksPer9Inn", "avg"]


def _slim(stat, keys):
    if not stat:
        return None
    out = {k: stat.get(k) for k in keys if stat.get(k) is not None}
    return out or None


def player_profile(pid, season_=None):
    """Full profile for one player: bio, season + career lines, situational
    splits, and Statcast pitch-type performance."""
    if not pid:
        return None
    season_ = season_ or SEASON
    ck = (str(pid), str(season_))
    c = _profile_cache.get(ck)
    if c and time.time() - c[0] < _PROFILE_TTL:
        return c[1]

    prof = {"id": pid}
    try:
        person = (_get(f"{BASE}/people/{pid}").get("people") or [{}])[0]
        prof.update({
            "name": person.get("fullName"),
    ...
    is_pitcher = (prof.get("position") or "").upper() in ("P", "SP", "RP")
    group = "pitching" if is_pitcher else "hitting"
    keys = _PIT_KEYS if is_pitcher else _HIT_KEYS

    prof["group"] = group
    prof["season"] = _slim(_stat_group(pid, "season", group, season_), keys)
    prof["career"] = _slim(_stat_group(pid, "career", group), keys)

    # situational splits — the ones people actually ask about
    splits = {}
    if is_pitcher:
        wanted = [("vs LHB", "vl"), ("vs RHB", "vr"), ("RISP", "risp"),
                  ("Home", "h"), ("Away", "a")]
    else:
        wanted = [("vs LHP", "vl"), ("vs RHP", "vr"), ("RISP", "risp"),
                  ("Bases loaded", "lo"), ("Home", "h"), ("Away", "a")]
    for label, code in wanted:
        st = _slim(_stat_group(pid, "statSplits", group, season_, code), keys)
        if st:
            splits[label] = st
    if splits:
        prof["splits"] = splits

    # last 10 games / recent form
    try:
        data = _get(f"{BASE}/people/{pid}/stats",
                    {"stats": "lastXGames", "group": group,
                     "season": season_, "limit": 10})
        sp = ((data.get("stats") or [{}])[0].get("splits") or [])
        recent = _slim(sp[0].get("stat"), keys) if sp else None
    except Exception:
        recent = None
    # if the API ignored the limit and handed back the season line, don't show it
    if recent and recent != prof.get("season"):
        prof["last_10"] = recent

    # year-by-year (career arc)
    try:
        data = _get(f"{BASE}/people/{pid}/stats",
                    {"stats": "yearByYear", "group": group})
        yrs = []
        for sp in ((data.get("stats") or [{}])[0].get("splits") or [])[-8:]:
            row = _slim(sp.get("stat"), keys)
            if row:
                row["season"] = sp.get("season")
                row["team"] = (sp.get("team") or {}).get("name")
                yrs.append(row)
        if yrs:
            prof["by_year"] = yrs
    except Exception:
    ...
    # Statcast: performance by pitch type (the part no basic box score has)
    try:
        import statcast as SC
        nm = prof.get("name")
        if nm:
            if is_pitcher:
                p = SC.pitcher_profile(nm)
                if p:
                    prof["statcast"] = p
            else:
                arse = SC.batter_arsenal()
                row = (arse or {}).get(nm.strip().lower())
                if row:
                    lg_ba = SC.league_stat_by_pitch("ba") or {}
                    lg_wh = SC.league_stat_by_pitch("whiff_pct") or {}
                    by_pitch = []
                    for pt, v in row.items():
                        if not v.get("pa"):
                            continue
                        by_pitch.append({
                            "pitch": pt, "pa": v.get("pa"), "ba": v.get("ba"),
                            "slg": v.get("slg"), "woba": v.get("woba"),
                            "whiff_pct": v.get("whiff_pct"),
                            "league_ba": lg_ba.get(pt),
                            "league_whiff_pct": lg_wh.get(pt),
                        })
                    by_pitch.sort(key=lambda x: -(x.get("pa") or 0))
                    if by_pitch:
                        prof["vs_pitch_type"] = by_pitch
    except Exception:
        pass

    if not prof.get("team") and prof.get("by_year"):
        prof["team"] = prof["by_year"][-1].get("team")
    _profile_cache[ck] = (time.time(), prof)
    return prof


def find_player(name):
    """Search MLB players by name -> [{id, name, team, position}]."""
    if not name:
        return []
    try:
        data = _get(f"{BASE}/people/search", {"names": name, "limit": 10})
        out = []
...
_ctx_map_cache = {}
_CTX_MAP_TTL = int(os.environ.get("MLB_CTX_TTL", "1800"))    # 30 min


def prop_context_map(date, game_id):
    """{(player_lower, stat_lower): (context, why)} for one game, CACHED.

    Book props carry the sportsbook's line but none of the model's reasoning.
    Building that reasoning means running the full projection pass (pitcher
    splits, Statcast arsenal, lineup lookups), which is far too slow to redo on
    every page load — so it's computed once per game and reused.
    """
    key = (str(game_id), str(date))
    c = _ctx_map_cache.get(key)
    if c and time.time() - c[0] < _CTX_MAP_TTL:
        return c[1]
    out = {}
    for fn in (get_props, get_batter_props):
        try:
            src = fn(date, game_id) or {}
        except Exception as e:
            print(f"[mlb] context map {fn.__name__} failed: {e}")
            continue
        for mp in (src.get("props") or []):
            ctx, why = mp.get("context"), mp.get("why")
            if not ctx and not why:
                continue
            k = ((mp.get("player") or "").strip().lower(),
                 (mp.get("label") or mp.get("stat") or "").strip().lower())
            out[k] = (ctx, why)
    _ctx_map_cache[key] = (time.time(), out)
    return out
