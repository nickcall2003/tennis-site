...
def league_teams(sport):
    ...
    per sport — used by the win-totals page to enumerate the league. ESPN's /teams
    endpoint paginates (~50/page), so we walk the pages; without this only the
    first ~50 teams came back (most of FBS was missing)."""
    import espn_provider as EP
    base = _BASE.get(sport)
    if not base:
        return []
    ...
    try:
        page = 1
        while page <= 12:                       # hard cap; ~150 FBS teams max
            data = EP._get(f"{base}/teams", {"limit": 100, "page": page}) or {}
