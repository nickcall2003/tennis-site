...
# Roster-change adjustment (transfers + recruiting), built by build_ncaaf_roster.py.
# Adds each team's `adj_sp` to its SP+ rating so portal moves / incoming talent
# actually shift the margin. OFF automatically if the file isn't present.
# The Action commits ncaaf_roster.json to the REPO ROOT (there's no /data on the
# GitHub runner), so we search several locations at load time and use whichever
# exists — the committed repo copy, the module's own directory, or /data.
_ROSTER_CANDIDATES = [
    p for p in [
        os.environ.get("NCAAF_ROSTER_PATH"),
        "/data/ncaaf_roster.json",
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "ncaaf_roster.json"),
        "ncaaf_roster.json",
    ] if p
]
# How much of the roster adjustment to apply early vs late in the season: full
# preseason (transfers are the story), fading as real games accrue and the
# in-season efficiency blend takes over. 1.0 = full weight always.
# How much of the roster adjustment to apply in the PRESEASON. SP+ preseason
# already includes transfers + returning production, so at full weight we'd
# double-count. Keep this low (a nudge for what SP+ underweights); the in-season
# efficiency blend and real results do the heavy lifting once games are played.
ROSTER_PRESEASON_W = float(os.environ.get("NCAAF_ROSTER_WEIGHT", "0.3"))
_roster = None
...
def _load_roster():
    """Load the roster-change file from the first candidate path that exists:
    {teams:{norm:{adj_sp, impact, key_in, ...}}}. Empty when no file — the
    adjustment is then simply 0 for every team."""
    global _roster
    if _roster is None:
        _roster = {"teams": {}}
        for p in _ROSTER_CANDIDATES:
            try:
                with open(p) as f:
                    _roster = json.load(f)
                _roster["_loaded_from"] = p
                break
            except Exception:
                continue
    return _roster
...
def _roster_adj(name):
    """Roster-change SP+ adjustment for a team (0.0 if none). Matched with the
    same normalizer/alias logic as SP+ so keys line up."""
    d = _load_roster()
    ...
    if not rec:
        return 0.0, None
    return float(rec.get("adj_sp") or 0.0) * ROSTER_PRESEASON_W, rec


def reload():
    global _data, _adv, _roster
    _data = None
    _adv = None
    _roster = None
    return _load()
...
def predict(home_name, away_name):
    ...
    # Roster-change adjustments (transfers + recruiting) shift each team's
    # effective SP+ up/down. FCS-baseline side gets no adjustment (it has no
    # roster data), which is correct.
    h_sp, a_sp = h["sp"], a["sp"]
    h_roster = a_roster = 0.0
    h_rinfo = a_rinfo = None
    if not (fcs_side == "home"):
        h_roster, h_rinfo = _roster_adj(home_name)
    if not (fcs_side == "away"):
        a_roster, a_rinfo = _roster_adj(away_name)
    # CAP the NET roster swing on the margin. SP+ preseason already bakes in
    # transfers/returning production, so the roster layer should only NUDGE the
    # spread, never dominate it. Without this cap, full-weight roster points stack
    # on top of an SP+ that already counted them (double-count) — or a one-sided
    # name-match miss (only one team found) — and produce absurd blowouts
    # (Miami -60, Fresno -20 over USC). We cap the net effect the roster layer can
    # have on the margin to +/- NCAAF_ROSTER_MARGIN_CAP points.
    net_roster = h_roster - a_roster
    cap = float(os.environ.get("NCAAF_ROSTER_MARGIN_CAP", "7.0"))
    net_roster = max(-cap, min(cap, net_roster))
    h_eff = h_sp + h_roster
    a_eff = a_sp + a_roster

    # Apply the CAPPED net roster effect to the margin (not the raw per-team sum),
    # so the spread moves by at most `cap` from roster changes.
    sp_margin = (h_sp - a_sp) + HOME_FIELD + net_roster
    ...
    out = {
        ...
        "home_rating": round(h_eff, 1),
        "away_rating": round(a_eff, 1),
        "home_rating_base": round(h_sp, 1),
        "away_rating_base": round(a_sp, 1),
        "confidence": confidence,
        "avg_total": total,
        "model": model,
    }
    if h_roster or a_roster:
        out["roster_adjustment"] = {"home": round(h_roster, 2),
                                    "away": round(a_roster, 2),
                                    "net_applied": round(net_roster, 2)}
    if adv_w > 0:
