...
_ROSTER_PATH = (os.environ.get("NCAAF_ROSTER_PATH")
                or ("/data/ncaaf_roster.json" if os.path.exists("/data/ncaaf_roster.json")
...
def _load_roster():
    """Load the roster-change file: {teams:{norm:{adj_sp, portal_pts,...}}}.
    Empty when no file — the adjustment is then simply 0 for every team."""
    global _roster
    if _roster is None:
        try:
