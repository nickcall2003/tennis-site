...
# Minor-tour tennis (ITF / Challenger / Futures / qualifiers) is the heaviest
# consumer of the odds quota — each is its own per-tournament sport key. We skip
# fetching market odds for them so the monthly budget goes to the main tours.
# Configurable: ODDS_SKIP_TENNIS is a comma-separated list of substrings to drop.
_SKIP_TENNIS = tuple(t.strip().lower() for t in os.environ.get(
    "ODDS_SKIP_TENNIS", "challenger,itf,futures,qual").split(",") if t.strip())


def _is_minor_tennis(key):
    k = (key or "").lower()
    return any(t in k for t in _SKIP_TENNIS)
...
def _active_sport_keys():
    ...
    try:
        import httpx
        r = httpx.get(f"{BASE}/sports", params={"apiKey": API_KEY, "all": "false"}, timeout=6)
        r.raise_for_status()
...
def get_odds(sport: str):
    ...
    for g in games:
        ...
        best_ml = {}     # team -> (price, book_title)  — best (highest) available
        books = 0
        for bk in g.get("bookmakers", []) or []:
            books += 1
            bk_title = bk.get("title") or bk.get("key") or "book"
            for mkt in bk.get("markets", []) or []:
                ...
                for oc in mkt.get("outcomes", []) or []:
                    ...
                    if mkey == "h2h" and nm and price is not None:
                        ...
                        # "best" = the price that pays the most for the bettor.
                        # Higher American number is always better (e.g. +150 >
                        # +130, and -105 > -120).
                        cur = best_ml.get(nm)
                        if cur is None or price > cur[0]:
                            best_ml[nm] = (price, bk_title)
        ...
        bh, ba = best_ml.get(home), best_ml.get(away)
        rec = {
            "home_team": home, "away_team": away,
            "ml_home": med(ml.get(home, [])), "ml_away": med(ml.get(away, [])),
            "ml_home_best": bh[0] if bh else None,
            "ml_home_book": bh[1] if bh else None,
            "ml_away_best": ba[0] if ba else None,
            "ml_away_book": ba[1] if ba else None,
            "spread_home": sp_h,
            "spread_away": sp_a,
    ...
    try:
        ...
        r = httpx.get(f"{BASE}/sports/{key}/odds",
                      params={"regions": "us", "markets": "outrights",
                              "oddsFormat": "american", "apiKey": API_KEY}, timeout=6)
        r.raise_for_status()
        data = r.json()
        _spend_one()
    except Exception as e:
...
def _active_tennis_keys():
    ...
    try:
        ...
        keys = [s["key"] for s in r.json()
                ...
                and not _is_minor_tennis(s.get("key", ""))]
    except Exception as e:
    ...
    if book is None:
        try:
            ...
            r = httpx.get(f"{BASE}/sports/{SPORT_KEY[sport]}/events",
                          params={"apiKey": API_KEY, "dateFormat": "iso"}, timeout=6)
            r.raise_for_status()
            book = {}
            for e in r.json() or []:
