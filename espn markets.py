...
def _devig_opposite(fav_ml):
    """Estimate the other side's moneyline from the favorite's, removing the
    favorite's implied edge and adding a touch of standard hold back. This is an
    ESTIMATE (ESPN only gives the favorite's price in `details` for baseball) —
    the user confirms/edits it on the bet form before saving."""
    fi = _ml_implied(fav_ml)
    dog = min(0.97, max(0.03, (1.0 - fi) + 0.022))
    if dog >= 0.5:
        return round(-100.0 * dog / (1.0 - dog))
    return round(100.0 * (1.0 - dog) / dog)


def _ml_from_details(details, home_abbr, away_abbr):
    """For baseball, ESPN's `details` is the favorite's moneyline, e.g. 'BAL -136'
    (a spread reads like 'LAL -5.5'). Returns ('home'|'away', ml) or None."""
    if not details:
        return None
    ...
    try:
        ...
        return None
    if abs(num) < 100:                 # that's a point spread, not a moneyline
        return None
    abbr = parts[0]
    if home_abbr and abbr == home_abbr:
        return ("home", num)
    if away_abbr and abbr == away_abbr:
        return ("away", num)
    return None
...
def game_markets(sport, yyyymmdd=None):
    ...
    for ev in data.get("events", []) or []:
        ...
        ml_home_src = ml_away_src = "espn"

        # ESPN often leaves homeTeamOdds.moneyLine null and only carries the
        # favorite's price in `details` (true for baseball). Recover it there and
        # estimate the underdog so both sides are tappable.
        if ml_home is None and ml_away is None:
            pm = _ml_from_details(odds.get("details"), home.get("abbr"), away.get("abbr"))
            if pm:
                side, fav_ml = pm
                dog_ml = _devig_opposite(fav_ml)
                if side == "home":
                    ml_home, ml_away = fav_ml, dog_ml
                    ml_away_src = "est"
                else:
                    ml_away, ml_home = fav_ml, dog_ml
                    ml_home_src = "est"

        # Resolve the home spread sign, anchored to the favorite.
        ...
        out.append({
            ...
            "ml_home_src": ml_home_src, "ml_away_src": ml_away_src,
            "total": total,
