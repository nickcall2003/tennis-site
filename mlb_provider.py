...
def get_props(date: dt.date, game_id: int):
    ...
    for side, pid, opp in (("home", home_pid, g["away"]["name"]),
        ...
        if proj:
            ...
            try:
                _lean = "UNDER" if proj.get("projection") is not None and line is not None \
                    and float(proj["projection"]) < float(line) else "OVER"
                _why = [f"Model projects {proj.get('projection')} K vs a {line} line \u2014 {_lean}."]
                if rate:
                ...
                if sc_ctx and "whiffs" in sc_ctx:
                    _why.append("Their whiff rate against his best pitch "
                                + ("supports" if _lean == "OVER" else "works against")
                                + " the over.")
                proj["why"] = " ".join(_why)
            except Exception:
                pass
