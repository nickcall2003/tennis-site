...
def _team_athlete_avgs(sport, team_id, season=None):
    """Return {athlete_name: {stat_key: season_avg}} for a team. Pass `season`
    (e.g. 2024) to fetch a specific season's averages — used as an early-season
    fallback when the current season has no games logged yet."""
    out = {}
    if not team_id:
        return out
    try:
        ...
        data = _get(_ATHLETE_STATS[sport].format(team=team_id), params)
    except Exception:
        return out
            ...
            for st in cat.get("stats", []) or []:
                ...
                if sname in wanted:
                    try:
                        ...
                        pass
        if vals:
            vals["_pos"] = ((ath.get("position") or {}) or {}).get("abbreviation") or ""
            out[nm] = vals
    return out
...
def get_props(sport: str, date: dt.date, game_id: str):
    ...
    # ADVANCED (best-effort): true defensive rating / pace / usage from stats.nba.com.
    # If that host blocks this server, everything below falls back to the ESPN
    # points-allowed proxy — no fabricated numbers, no broken page.
    _adv_team, _adv_usage = None, None
    try:
        if sport in ("nba", "wnba"):
            import hoops_advanced as HA
            _adv_team = HA.team_advanced(sport)
            _adv_usage = HA.player_usage(sport)
    except Exception:
        _adv_team, _adv_usage = None, None

    def _adv_for(name):
        if not _adv_team or not name:
            return None
        n = str(name).upper()
        for k, v in _adv_team.items():
    ...
    for side in ("home", "away"):
        ...
        opp_def = _def.get(opp)                 # points the OPPONENT allows (ESPN proxy)
        team_off, opp_off = _off.get(side), _off.get(opp)
        # Prefer TRUE advanced numbers (defensive rating + pace) when reachable;
        # otherwise fall back to the ESPN points-for/against proxy.
        adv_opp = _adv_for(g[opp].get("name") or opp_abbr)
        adv_own = _adv_for(g[side].get("name") or tabbr)
        adv_used = False
        lg_drtg = {"nba": 113.0, "wnba": 101.0}.get(sport, 105.0)
        lg_pace = {"nba": 99.0, "wnba": 95.0}.get(sport, 97.0)
        if adv_opp and adv_opp.get("def_rtg"):
            def_factor = _clamp(float(adv_opp["def_rtg"]) / lg_drtg, 0.88, 1.12)
            adv_used = True
        else:
            def_factor = _clamp((opp_def / _LEAGUE_PPG) if opp_def else 1.0, 0.88, 1.12)
        if adv_opp and adv_opp.get("pace") and adv_own and adv_own.get("pace"):
            pace_factor = _clamp(
                (float(adv_own["pace"]) + float(adv_opp["pace"])) / (2 * lg_pace), 0.90, 1.10)
            adv_used = True
        else:
        ...
        # EARLY-SEASON FALLBACK: in Week 1 (and until players log games) the
        # current-season averages are empty, so there'd be no model projections
        # next to the book lines. Fall back to last season's per-game averages so
        # a projection still shows. Self-heals: once current-season data exists,
        # this branch is skipped.
        if not avgs:
            import datetime as _dt2
            _last = _dt2.date.today().year - (1 if _dt2.date.today().month >= 8 else 2)
            try:
                avgs = _team_athlete_avgs(sport, team_id, season=_last) or {}
            except Exception:
                pass
        # League-average production allowed to each position bucket (the neutral
        # baseline the DvP numbers are compared against).
        opp_name = g[opp].get("name") or opp_abbr
        _LG_DVP = {
            "nba": {"pts": 24.0, "reb": 10.5, "ast": 6.0, "fg3m": 3.2},
            "wnba": {"pts": 18.0, "reb": 8.5, "ast": 4.5, "fg3m": 2.0},
        }.get(sport)

        def _dvp_factor(pos, stat_key):
            """How much MORE/LESS the opponent allows to this player's position for
            this stat vs. league average. Real data only \u2014 returns (1.0, None) when
            the uploaded snapshot isn't there."""
            if not _LG_DVP or not pos:
                return 1.0, None
            try:
                import hoops_advanced as HA
                row = HA.defense_vs_position(sport, opp_name, pos)
            except Exception:
                row = None
            if not row:
                return 1.0, None
            skey = {"points": "pts", "rebounds": "reb",
                    "assists": "ast", "threes": "fg3m"}.get(stat_key)
            allowed = row.get(skey) if skey else None
            base = _LG_DVP.get(skey) if skey else None
            if not allowed or not base:
                return 1.0, None
            f = _clamp(float(allowed) / float(base), 0.85, 1.15)
            return f, {"allowed": round(float(allowed), 1), "pos": pos, "stat": skey}

        for pname, vals in avgs.items():
            ppos = vals.get("_pos") or ""
            for stat_key, label in _PROP_STATS[sport]:
                ...
                # DEFENSE VS POSITION drives the matchup adjustment when we have it;
                # it applies to EVERY stat (pts/reb/ast/3s), which the team-wide
                # points-allowed proxy never could. Fall back to the team factor.
                dvp_f, dvp_info = _dvp_factor(ppos, stat_key)
                if dvp_info:
                    matchup_f = dvp_f
                else:
                    matchup_f = def_factor if scoring else 1.0
                adj_rate = rate * pace_factor * matchup_f
                ...
                proj["factors"] = {
                    ...
                    "advanced": adv_used,
                    "opp_def_rtg": (adv_opp or {}).get("def_rtg"),
                    "pace": (adv_own or {}).get("pace"),
                }
                usg = None
                if _adv_usage:
                    u = _adv_usage.get(str(pname).lower())
                    if u and u.get("usg_pct") is not None:
                        usg = round(float(u["usg_pct"]) * (100 if float(u["usg_pct"]) < 1 else 1), 1)
                        proj["factors"]["usage_pct"] = usg
                _da = proj["factors"]["def_adj_pct"]
                _pa = proj["factors"]["pace_adj_pct"]
                bits = [f"season avg {round(rate,1)}"]
                if dvp_info:
                    # the real "opponent allows X to this position" line
                    proj["factors"]["dvp_allowed"] = dvp_info["allowed"]
                    proj["factors"]["dvp_adj_pct"] = round((dvp_f - 1) * 100, 1)
                    _pos_lbl = {"G": "guards", "F": "forwards", "C": "centers",
                                "PG": "guards", "SG": "guards", "SF": "forwards",
                                "PF": "forwards"}.get(str(ppos).upper(), f"{ppos}s")
                    bits.append(f"{opp_abbr} allows {dvp_info['allowed']} "
                                f"{dvp_info['stat']} to {_pos_lbl} "
                                f"({'%+.0f' % ((dvp_f - 1) * 100)}%)")
                elif adv_used and (adv_opp or {}).get("def_rtg"):
                    bits.append(f"vs {opp_abbr} D-Rtg {round(float(adv_opp['def_rtg']),1)}"
                                + (f" ({'%+.0f' % _da}%)" if scoring and _da else ""))
                elif opp_def:
                    bits.append(f"vs {opp_abbr} D (allows {round(opp_def,1)} ppg{', ' + ('%+.0f' % _da) + '%' if scoring and _da else ''})")
                if abs(_pa) >= 0.5:
                    bits.append(f"pace {'%+.0f' % _pa}%")
                if usg:
                    bits.append(f"{usg}% usage")
...
def get_prop_history(sport, date, game_id, player_name, stat, line):
    ...
    for ev in rows[-10:]:
        ...
        try:
            ...
            opp = ""
        # real date (so L5/L10 recency ordering is correct) + home/away flag
        gdate = ""
        try:
            gdate = (ev_meta.get("gameDate") or "")[:10]
        except Exception:
            gdate = ""
        at_home = None
        try:
            ha = ev_meta.get("atVs") or ev_meta.get("homeAwaySymbol") or ""
            if ha:
                at_home = str(ha).strip().lower() not in ("@", "at")
            elif ev_meta.get("homeTeamId") and ev_meta.get("team", {}).get("id"):
                at_home = str(ev_meta["homeTeamId"]) == str(ev_meta["team"]["id"])
        except Exception:
            at_home = None
        # minutes for this game (drives the minutes-trend factor)
        mins = None
        try:
            mcol = None
            for i in range(max(len(labels), len(names))):
                lab = labels[i] if i < len(labels) else ""
                nm2 = names[i] if i < len(names) else ""
                if lab == "min" or nm2 == "minutes":
                    mcol = i
                    break
            if mcol is not None and mcol < len(stats):
                mins = float(str(stats[mcol]).replace(",", ""))
        except (ValueError, TypeError):
            mins = None
        games.append({"date": gdate, "opp": opp, "value": val, "home": at_home, "min": mins})
...
def final_player_stats(sport, date, game_id):
    """{(player_lower, stat_label_lower): value} for a FINAL game — used to grade
    the player-prop projections. Returns {} if the game isn't final or on any error.
    Reads the same boxscore ESPN already serves; no new data source."""
    out = {}
    url = SUMMARY.get(sport)
    if not url:
        return out
    ...
    try:
        ...
        pass
    # boxscore.players[] -> statistics[] -> {names/labels/keys, athletes[]:{athlete, stats[]}}
    want = {"pts": "points", "reb": "rebounds", "ast": "assists", "3pt": "3-pointers",
            "min": "minutes"}
    try:
        for team in ((data.get("boxscore") or {}).get("players") or []):
            for grp in (team.get("statistics") or []):
                labels = [str(x).lower() for x in (grp.get("labels") or [])]
                names = [str(x).lower() for x in (grp.get("names") or [])]
                for ath in (grp.get("athletes") or []):
                    nm = ((ath.get("athlete") or {}).get("displayName") or "").strip().lower()
                    stats = ath.get("stats") or []
                    if not nm or not stats:
                        continue
                    for i in range(min(len(stats), max(len(labels), len(names)))):
                        lab = labels[i] if i < len(labels) else ""
                        nam = names[i] if i < len(names) else ""
                        key = want.get(lab) or want.get(nam)
                        if not key:
                            continue
                        raw = str(stats[i]).strip()
                        # "3PT" comes as made-attempted (e.g. "4-9") -> take made
                        if "-" in raw and key == "3-pointers":
                            raw = raw.split("-")[0]
                        try:
                            out[(nm, key)] = float(raw)
                        except (TypeError, ValueError):
                            continue
        # Pts+Reb+Ast combo (a market we offer) — derive it from the parts
        combos = {}
        for (nm, key), v in out.items():
            if key in ("points", "rebounds", "assists"):
                combos.setdefault(nm, {})[key] = v
        for nm, parts in combos.items():
            if len(parts) == 3:
                out[(nm, "pts+reb+ast")] = sum(parts.values())
    except Exception:
        return out
    return out
...
# Same idea as the MLB profiles, for the ESPN-backed sports. Every section is
# parsed defensively and simply omitted if ESPN doesn't return it — a profile
# never shows a stat we didn't actually receive.

_COMMON_V3 = {
    "nba": "https://site.web.api.espn.com/apis/common/v3/sports/basketball/nba",
    "wnba": "https://site.web.api.espn.com/apis/common/v3/sports/basketball/wnba",
...
_profile_cache_espn = {}
_PROFILE_TTL_ESPN = 1800


def _stat_table(block):
    """ESPN returns stats as parallel name/value arrays in several shapes.
    Pull out {label: value} for whatever shape we got, or None."""
    if not isinstance(block, dict):
        return None
    names = (block.get("labels") or block.get("displayNames")
             or block.get("names") or [])
    vals = block.get("displayValues") or block.get("values") or block.get("stats") or []
    if names and vals:
        out = {}
        for i in range(min(len(names), len(vals))):
            ...
            out[str(k)] = v
        return out or None
    # some shapes are a list of {name, displayValue}
    stats = block.get("stats") or block.get("statistics")
    if isinstance(stats, list):
        out = {}
        for st in stats:
            if not isinstance(st, dict):
                continue
            k = st.get("displayName") or st.get("name") or st.get("shortDisplayName")
...
def player_profile(sport, pid):
    """Bio + season/career stats + splits + recent form for an ESPN athlete."""
    base = _COMMON_V3.get(sport)
    if not base or not pid:
        return None
    ck = (sport, str(pid))
    c = _profile_cache_espn.get(ck)
    if c and time.time() - c[0] < _PROFILE_TTL_ESPN:
        return c[1]

    prof = {"id": pid, "sport": sport, "_sections": []}

    # --- bio ---
    try:
        d = _get(f"{base}/athletes/{pid}") or {}
        a = d.get("athlete") or d
        prof.update({
            "name": a.get("displayName") or a.get("fullName"),
            "position": ((a.get("position") or {}).get("abbreviation")
                         or (a.get("position") or {}).get("displayName")),
            "team": ((a.get("team") or {}).get("displayName")
                     or (a.get("team") or {}).get("name")),
            "jersey": a.get("jersey"),
            "age": a.get("age"),
            "height": a.get("displayHeight") or a.get("height"),
            "weight": a.get("displayWeight") or a.get("weight"),
            "headshot": ((a.get("headshot") or {}).get("href")),
            "experience": ((a.get("experience") or {}).get("years")),
        })
        if prof.get("name"):
            prof["_sections"].append("bio")
    except Exception as e:
        prof["_bio_error"] = str(e)[:100]

    # --- season + career stat tables ---
    try:
        d = _get(f"{base}/athletes/{pid}/stats") or {}
        tables = []
        for cat in (d.get("categories") or []):
            t = _stat_table(cat)
            if t:
                tables.append({"label": cat.get("displayName") or cat.get("name"),
                               "stats": t})
        # some responses nest under splits
        for sp in ((d.get("splits") or {}).get("categories") or []):
            t = _stat_table(sp)
            if t:
                tables.append({"label": sp.get("displayName") or sp.get("name"),
                               "stats": t})
        if tables:
            prof["stats"] = tables
            prof["_sections"].append("stats")
    except Exception as e:
        prof["_stats_error"] = str(e)[:100]

    # --- situational splits (home/away, vs conference, etc.) ---
    try:
        d = _get(f"{base}/athletes/{pid}/splits") or {}
        hdr = {"labels": d.get("labels"), "displayNames": d.get("displayNames"),
               "names": d.get("names")}
        rows, seen = [], set()
        for cat in (d.get("splitCategories") or d.get("categories") or []):
            for sp in (cat.get("splits") or []):
                t = _stat_table({"labels": hdr["labels"],
                                 "displayNames": hdr["displayNames"],
                                 "names": hdr["names"],
                                 "displayValues": sp.get("stats")})
                if not t:
                    continue
                sig = tuple(sorted(t.items()))
                if sig in seen:        # ESPN repeats the overall line per category
                    continue
                seen.add(sig)
                lbl = sp.get("displayName") or sp.get("abbreviation") or ""
                rows.append({"label": lbl,
                             "group": cat.get("displayName") or cat.get("name"),
                             "stats": t})
        if rows:
            # the first unique row is the player's overall season line
            rows[0]["label"] = "Season"
            rows[0].pop("group", None)
            prof["season"] = rows[0]
            extra = [r for r in rows[1:] if r.get("label")]
            if extra:
                prof["splits"] = extra[:8]
            prof["_sections"].append("splits")
    except Exception as e:
        prof["_splits_error"] = str(e)[:100]

    # --- recent form from the game log we already use for projections ---
    try:
        url = GAMELOG.get(sport)
        if url:
            d = _get(url.format(pid=pid)) or {}
            events = []
            seasons = (d.get("seasonTypes") or [])
            for s in seasons:
                for cat in (s.get("categories") or []):
                    for ev in (cat.get("events") or []):
                        events.append(ev)
            if events:
                prof["recent_games"] = len(events)
                prof["_sections"].append("gamelog")
    except Exception as e:
        prof["_gamelog_error"] = str(e)[:100]

    _profile_cache_espn[ck] = (time.time(), prof)
    return prof


def find_athlete(sport, name):
    """Resolve a player name to an ESPN athlete id using today's rosters."""
    try:
        aid = _athlete_id_by_name(sport, name)
        return [{"id": aid, "name": name}] if aid else []
    except Exception:
        return []


_CAREER_BACK_ESPN = int(os.environ.get("ESPN_CAREER_SEASONS", "5"))


def _season_line(sport, pid, yr):
    """One season's overall line from ESPN's splits endpoint (the shape we know
    works). Returns {label: value} or None."""
    base = _COMMON_V3.get(sport)
    if not base:
        return None
    try:
        d = _get(f"{base}/athletes/{pid}/splits", {"season": yr}) or {}
    except Exception:
        return None
    hdr = {"labels": d.get("labels"), "displayNames": d.get("displayNames"),
           "names": d.get("names")}
    for cat in (d.get("splitCategories") or d.get("categories") or []):
        for sp in (cat.get("splits") or []):
            t = _stat_table({"labels": hdr["labels"],
                             ...
                             "displayValues": sp.get("stats")})
            if t:
                return t          # first row is the overall season line
    return None


def career_profile(sport, pid, back=None):
    """Season-by-season history plus a games-weighted career average.

    Honest note: ESPN's per-season splits give AVERAGES, not totals, so a career
    line here is a weighted average across the seasons we loaded — not a lifetime
    total. The span is returned so the UI can label it accurately.
    """
    back = back or _CAREER_BACK_ESPN
    now = dt.date.today()
    # WNBA runs inside one calendar year; NBA/NFL/NHL seasons are labelled by the
    # year they end in.
    cur = now.year if sport == "wnba" else (now.year + 1 if now.month >= 9 else now.year)
    rows, used = [], []
    for yr in range(cur, cur - back, -1):
        line = _season_line(sport, pid, yr)
        if not line:
            continue
        rows.append({"season": str(yr), "stats": line})
        used.append(yr)
    if not rows:
        return None
    # games-weighted average across seasons (averages can't simply be summed)
    def _is_games_key(k):
        kl = str(k).lower()
        return "game" in kl or kl in ("gp", "g")
        ...
        for k, v in st.items():
            if _is_games_key(k):
                try:
                    ...
                    return None
        return None
    is_games = _is_games_key

    def parts(v):
        """'4.8-10.7' -> [4.8, 10.7]; '44.4' -> [44.4]; else None."""
        try:
    ...
    for r in rows:
        ...
        for k, v in r["stats"].items():
            ...
            if not ps:
                continue
            cur = totals.setdefault(k, [0.0] * len(ps))
            if len(cur) < len(ps):
                cur += [0.0] * (len(ps) - len(cur))
            if is_games(k):
                for i, x in enumerate(ps):
                    cur[i] += x                    # games played SUM
                weights[k] = 1.0
            else:
                for i, x in enumerate(ps):
                    cur[i] += x * g                # everything else is weighted
                weights[k] = weights.get(k, 0.0) + g
    career = {}
    for k, tot in totals.items():
        w = weights.get(k) or 1.0
        vals = [(t if w == 1.0 else t / w) for t in tot]
        fmt = []
        for val in vals:
            fmt.append(str(int(round(val))) if abs(val - round(val)) < 1e-9
                       else str(round(val, 1)))
        career[k] = "-".join(fmt)
    return {"career": career, "by_season": rows,
            "seasons_covered": sorted(used),
            "span": f"{min(used)}\u2013{max(used)}" if len(used) > 1 else str(used[0]),
            "basis": "games-weighted average of the seasons loaded"}
