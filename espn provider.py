...
def _stat_table(block):
    ...
    if not isinstance(block, dict):
        return None
    names = (block.get("names") or block.get("labels")
             or block.get("displayNames") or [])
    ...
    try:
        ...
        for cat in (d.get("splitCategories") or d.get("categories") or []):
            for sp in (cat.get("splits") or []):
                t = _stat_table({"names": d.get("names") or d.get("labels"),
                                 "displayValues": sp.get("stats")})
                if t:
                    rows.append({"label": sp.get("displayName") or sp.get("abbreviation"),
