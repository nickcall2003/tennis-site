...
SEASON-AVAILABILITY GUARD
A projection is only published when it's real: the schedule must have enough
games (so we don't project a handful of preseason exhibitions as the season),
and an unstarted season with no ratings at all is suppressed (otherwise every
team lands on the identical coin-flip number — e.g. every NHL team at 42.0).
Suppressed sports show the frontend's honest "fills in once the season starts"
empty state instead.
"""
...
# Minimum games before we trust a season projection. Below this we assume the
# regular-season schedule isn't posted yet (only preseason) and publish nothing.
_MIN_SCHED = {"nfl": 12, "ncaaf": 8, "nba": 40, "ncaab": 15, "nhl": 40, "mlb": 30}
...
def _season_year(sport, today=None):
    """The ESPN season year to request for a sport's REGULAR season. NFL/CFB/MLB
    are keyed by their starting (calendar) year; NBA/NHL/CBB span two years and
    ESPN keys them by the ending year."""
    ...
    if sport == "mlb":
        return y
    # nfl / ncaaf: fall-to-winter season, named by the starting (fall) year
    return y - 1 if m <= 2 else y


def _full_schedule(sport, team_id, season=None):
    """Every event on a team's REGULAR-season ESPN schedule — played and upcoming —
    with opponent id/name, home/away, and result if final.

    We request seasontype=2 (regular season) explicitly. Without it, ESPN returns
    whatever phase is current, which in August is PRESEASON — that's why NFL showed
    only 3 games and NBA 5. If the regular season isn't posted for the computed
    year (or the year convention is off), we fall back to the default fetch so we
    never regress below the old behavior."""
    ...
    def _fetch(params):
        try:
            return EP._get(url, params) or {}
        except Exception:
            return {}

    yr = season or _season_year(sport)
    data = _fetch({"season": yr, "seasontype": 2})
    events = data.get("events") or []
    if len(events) < 4:
        alt = _fetch({} if season is None else {"season": season})
        alt_events = alt.get("events") or []
        if len(alt_events) > len(events):
            events = alt_events
...
def league_teams(sport):
    """All teams in a league: [{team_id, name, abbr, logo}]. One cached fetch per
    sport. For college, ESPN's /teams endpoint spans EVERY division (FBS, FCS,
    D-II, D-III), so we pass the top-division group id (ncaaf FBS=80, ncaab D-I=50)
    and page within it — otherwise D-II/D-III schools leak into the list."""
    import espn_provider as EP
    base = _BASE.get(sport)
    if not base:
        return []
    ...
    if cached is not None:
        return cached
    _GROUP = {"ncaaf": "80", "ncaab": "50"}   # ESPN group ids for top division
    out, seen = [], set()
    try:
        page = 1
        while page <= 12:                       # hard cap
            params = {"limit": 100, "page": page}
            if sport in _GROUP:
                params["groups"] = _GROUP[sport]
            data = EP._get(f"{base}/teams", params) or {}
            league = ((data.get("sports") or [{}])[0].get("leagues") or [{}])[0]
            groups = league.get("teams") or []
            if not groups:
                break
            for wrap in groups:
                t = wrap.get("team") or {}
            ...
            page_count = league.get("pageCount") or data.get("pageCount")
            if not page_count or page >= int(page_count):
                break
            page += 1
    except Exception:
        pass
...
def project_team(sport, team_id, team_name, line=None, season=None):
    ...
    schedule context layered on top of the base rating model. Returns None when
    the projection isn't credible enough to publish (see the season guard)."""
    import espn_elo
    ...
    # --- season-availability / credibility guard -------------------------
    # Don't publish a projection that isn't real. Two cases we suppress (return
    # None -> the league endpoint drops the team; if ALL teams drop, the frontend
    # shows its honest "fills in once the season starts" empty state):
    #   1. Too few games -> the regular-season schedule isn't posted yet and
    #      we're only seeing preseason (NFL "3-0", NBA "5-0").
    #   2. Season hasn't started AND we can't rate a single game -> every team is
    #      an identical coin flip, e.g. every NHL team at 42.0. (In-season sports
    #      keep going even if unrated, because their PLAYED results are real.)
    n_games = len(games)
    n_played = sum(1 for g in games if g.played)
    if n_games < _MIN_SCHED.get(sport, 8):
        return None
    if n_played == 0 and missing_ratings >= n_games:
        return None
...
def _project_ncaaf_cfbd(lines=None):
    """CFB win totals built entirely from CollegeFootballData: the authoritative
    FBS team list and full regular-season schedules, rated by SP+ — all keyed on the
    same school names, so there's no cross-source ID matching to get wrong, and the
    team list is exactly FBS (no D-II/FCS leakage, no missing teams). Returns None if
    CFBD isn't available, so the ESPN path can take over."""
    try:
        import ncaaf_stats as CF
        import ncaaf_provider as NP
    except Exception:
        return None
    try:
        if not CF.has_key():
        ...
        return None
    teams = CF.fbs_teams()
    sched = CF.team_schedules()
    if not teams or not sched:
        return None
    fbs = {t["name"] for t in teams}
    lines = lines or {}
    out = []
    for t in teams:
        name = t["name"]
        rows = sched.get(name) or []
        if len(rows) < _MIN_SCHED.get("ncaaf", 8):
            continue
        games, rated = [], 0
        for r in rows:
            opp, home = r["opponent"], r["home"]
            try:
                pred = NP.predict(name, opp) if home else NP.predict(opp, name)
            except Exception:
                pred = None
            if pred and pred.get("prob_home") is not None:
                ph = float(pred["prob_home"])
                p = ph if home else (1.0 - ph)
                rated += 1
            elif opp not in fbs:
                p = 0.92 if home else 0.88      # FBS vs a non-FBS opponent
                rated += 1
            else:
                p = 0.5                          # two FBS teams, neither SP+-rated
            games.append(win_totals.Game(opponent=opp, win_prob=p, home=home,
                                         played=r["completed"], won=r["won"], date=r["date"]))
        n_played = sum(1 for g in games if g.played)
        if n_played == 0 and rated == 0:
            continue                             # unstarted + wholly unrated
        proj = win_totals.project(name, games, line=lines.get(name))
        d = win_totals.to_dict(proj)
        d["model_source"] = "efficiency"
        d["model"] = "cfbd+sp+"
        d["games_no_rating"] = len(games) - rated
        d["sos"] = None
        d["sos_rank_hint"] = None
        d["rating"] = None
        out.append(d)
    if not out:
        return None
    out.sort(key=lambda p: p["expected_wins"], reverse=True)
    return {"sport": "ncaaf", "count": len(out), "teams": out, "has_lines": False}
...
def project_league(sport, teams=None, lines=None):
    ...
    # CFB: enumerate + schedule from CFBD (authoritative FBS), not ESPN's
    # division-mixed /teams endpoint. Falls back to ESPN if CFBD is unavailable.
    if sport == "ncaaf" and teams is None:
        cfb = _project_ncaaf_cfbd(lines)
        if cfb is not None:
            return cfb
    lines = lines or {}
    if teams is None:
