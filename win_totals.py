...
TWO WAYS TO PROJECT, AND WHY THE SOFT ONE IS RIGHT
  ...
  * Expected wins: sum the win probabilities. Twelve 51% games = 6.12 expected
  ...
  We show both: expected wins as the projection, and the hard W/L marks as the
  game-by-game readout, because people want to see the walk-through.
...
class WinTotalProjection:
    ...
    projected_record: str           # hard W/L count as "10-2"
    games_total: int
    games_played: int
...
def project(team: str, games: list, line: float | None = None,
    ...
    # hard record: played results as-is, upcoming marked by >=0.5
    proj_w = actual_wins + sum(1 for g in upcoming if _clip(g.win_prob) >= 0.5)
    proj_l = len(games) - proj_w
    ...
    return WinTotalProjection(
        ...
        games=[_game_view(g) for g in games],
    )


def _game_view(g: Game):
    p = _clip(g.win_prob)
    if g.played:
