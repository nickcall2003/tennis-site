...
def chat_diag():
    """Minimal Anthropic call to surface the real failure (status + body)."""
    key = _get_key()
    if not key:
    ...
    try:
        ...
        r = httpx.post("https://api.anthropic.com/v1/messages",
                       headers={"x-api-key": key, "anthropic-version": "2023-06-01",
                                "content-type": "application/json"},
                       json={"model": _MODEL, "max_tokens": 16,
                             "messages": [{"role": "user", "content": "ping"}]},
                       timeout=20.0)
        return {"ok": r.status_code == 200, "status": r.status_code,
                "model": _MODEL, "key_tail": key[-4:], "body": r.text[:500]}
    except Exception as e:
        return {"ok": False, "error": str(e)[:500], "model": _MODEL, "key_tail": key[-4:]}
