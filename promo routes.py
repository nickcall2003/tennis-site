    ...
    try:
        ...
        if prof:
            ...
            if car:
                ...
                if hist:
                    # replace rather than append: the profile's own splits already
                    # contain the current season and would render twice
                    prof["splits"] = hist
...
@router.get("/api/ncaaf/diag")
def ncaaf_diag(year: int | None = None, token: str = "",
               authorization: str | None = Header(None)):
    """Which CollegeFootballData endpoints answer, with counts + field names."""
    ok = bool(_CRON_TOKEN) and (token or "").strip() == _CRON_TOKEN
    if not ok and not _is_admin(authorization):
        return {"error": "forbidden"}
    try:
        import ncaaf_stats as C
        return C.status(year)
    except Exception as e:
        import traceback
        return {"error": str(e)[:200], "trace": traceback.format_exc()[-400:]}


@router.get("/api/ncaaf/team")
def ncaaf_team(team: str, year: int | None = None):
    """Public: one college football team's SP+ and advanced profile."""
    try:
        import ncaaf_stats as C
        return C.team_profile(team, year) or {"error": "not found"}
    except Exception as e:
        return {"error": str(e)[:200]}


@router.get("/api/ncaaf/matchup")
def ncaaf_matchup(home: str, away: str, year: int | None = None):
    """Public: SP+ / advanced-stat comparison for one college football matchup."""
    try:
        import ncaaf_stats as C
        return C.matchup(home, away, year) or {"error": "not found"}
    except Exception as e:
        return {"error": str(e)[:200]}


@router.get("/api/ncaaf/refresh-sp")
def ncaaf_refresh_sp(year: int | None = None, token: str = "",
                     authorization: str | None = Header(None)):
    """Rebuild the SP+ ratings file from the live CFBD API and hot-reload the
    predictor. Safe to run daily; ratings move weekly in season."""
    ok = bool(_CRON_TOKEN) and (token or "").strip() == _CRON_TOKEN
    if not ok and not _is_admin(authorization):
        return {"error": "forbidden"}
    try:
        import ncaaf_stats as C
        return C.refresh_sp_file(year)
    except Exception as e:
        import traceback
        return {"error": str(e)[:200], "trace": traceback.format_exc()[-400:]}


@router.get("/api/ncaab/diag")
def ncaab_diag(year: int | None = None, token: str = "",
               authorization: str | None = Header(None)):
    """What the Torvik feed returns from this server: URL, mode, columns, sample."""
    ok = bool(_CRON_TOKEN) and (token or "").strip() == _CRON_TOKEN
    if not ok and not _is_admin(authorization):
        return {"error": "forbidden"}
    try:
        import ncaab_stats as C
        return C.status(year)
    except Exception as e:
        import traceback
        return {"error": str(e)[:200], "trace": traceback.format_exc()[-400:]}
...
@router.get("/api/ncaab/refresh-ratings")
def ncaab_refresh(year: int | None = None, token: str = "",
                  authorization: str | None = Header(None)):
    """Rebuild the adjusted-efficiency file the NCAAB predictor reads."""
    ok = bool(_CRON_TOKEN) and (token or "").strip() == _CRON_TOKEN
    if not ok and not _is_admin(authorization):
        return {"error": "forbidden"}
    try:
        import ncaab_stats as C
        return C.refresh_ratings_file(year)
    except Exception as e:
        import traceback
        return {"error": str(e)[:200], "trace": traceback.format_exc()[-400:]}


@router.get("/api/odds/quota")
def odds_quota():
    """Public: how much of the monthly odds allowance is left. Uses the counters
    the API returns on each call — no request is spent to read this."""
    try:
        import odds_api
        return {
            "enabled": odds_api.enabled(),
            "remaining_this_month": odds_api._quota.get("remaining"),
            "used_this_month": odds_api._quota.get("used"),
            "calls_today": odds_api._spend.get("count"),
            "daily_ceiling": odds_api._DAILY_MAX,
            "note": ("Each call requests h2h+spreads+totals. The Odds API bills "
                     "per market per region, so one call costs 3 credits here."),
        }
    except Exception as e:
        return {"error": str(e)[:200]}


@router.get("/api/{sport}/market/{game_id}")
def market_lines(sport: str, game_id: str, date: str | None = None):
    """Public: the SPORTSBOOK's moneyline, spread and total for one game.

    Reads the odds cache that's already populated for model comparison, so this
    endpoint never spends an API request of its own.
    """
    try:
        import odds_api
        if not odds_api.enabled():
            return {"available": False, "reason": "odds API not configured"}
        book = odds_api.get_odds(sport) or {}
        if not book:
            return {"available": False, "reason": "no odds cached for this league"}
        # resolve our game -> the odds record by team names
        home = away = None
        try:
            import main as _m
            target = _m.dt.date.fromisoformat(date) if date else _m.dt.date.today()
            g = None
            if sport == "mlb":
                from mlb_provider import get_game
                g = get_game(target, int(game_id))
            else:
                import espn_provider as EP
                g = EP.get_game(sport, game_id, target)
            if g:
                home = ((g.get("home") or {}).get("name")
                        or (g.get("home") or {}).get("team") or g.get("home_name"))
                away = ((g.get("away") or {}).get("name")
                        or (g.get("away") or {}).get("team") or g.get("away_name"))
        except Exception as e:
            return {"available": False, "reason": f"game lookup failed: {str(e)[:80]}"}
        if not home or not away:
            return {"available": False, "reason": "could not resolve team names"}
        key = odds_api._norm(home) + "|" + odds_api._norm(away)
        rec = book.get(key)
        if not rec:
            return {"available": False, "reason": "no market posted for this game",
                    "looked_for": key}
        return {"available": True, "home": rec.get("home"), "away": rec.get("away"),
                "moneyline": rec.get("h2h"), "spread": rec.get("spread"),
                "total": rec.get("total"), "books": rec.get("books"),
...
def _edge(model_prob, market_odds):
    """Model probability minus the market's implied probability, in points."""
    imp = _implied(market_odds)
    if imp is None or model_prob is None:
        return None
    try:
        return round((float(model_prob) - imp) * 100, 1)
    except (TypeError, ValueError):
        return None


@router.get("/api/lines")
def full_lines(team: str, sport: str | None = None, days: int = 3):
    """Model AND market prices for moneyline, spread and total on one game, with
    the edge on each. Built for the Discord bot.

    Market data comes from the odds cache that's already populated, so this costs
    no extra API requests.
    """
    try:
        import main as _m
        needle = (team or "").strip().lower()
        for suf in (" ml", " moneyline", " spread", " total", " over", " under"):
            if needle.endswith(suf):
                needle = needle[: -len(suf)].strip()
        if not needle:
            return {"found": False, "error": "no team provided"}
        days = max(1, min(int(days or 1), 7))
        today = _m.dt.date.today()
        for off in range(days):
            target = today + _m.dt.timedelta(days=off)
            try:
                _m._ensure_day(target)
                games = _m._games_for(target) if hasattr(_m, "_games_for") else None
            except Exception:
                games = None
            plays = []
            try:
                plays = _m._gather_plays(target) or []
            except Exception:
                plays = []
            for p in plays:
                if sport and p.get("sport") != sport:
                    continue
                bits = [p.get("match"), p.get("pick"), p.get("home"), p.get("away"),
                        p.get("home_name"), p.get("away_name")]
                hay = " ".join(str(b) for b in bits if b).lower()
                if needle not in hay:
                    continue
                out = {"found": True, "date": target.isoformat(),
                       "sport": p.get("sport"), "match": p.get("match"),
                       "event_time": p.get("event_time"), "model": {}, "market": {},
                       "edges": {}}
                # ---- model side ----
                # plays carry the pick, not the full game object, so fetch the
                # game and build its model lines the same way the site does
                lines = p.get("lines") or {}
                if not lines:
                    try:
                        gid = p.get("id") or p.get("game_id")
                        if p.get("sport") == "mlb" and gid:
                            from betting import mlb_lines as _ml
                            from mlb_provider import get_game as _mg
                            g = _mg(target, int(gid))
                            if g and g.get("exp_runs_home") is not None:
                                lines = _ml(g["exp_runs_home"],
                                            g["exp_runs_away"]) or {}
                        elif gid:
                            import espn_provider as _EP
                            from betting import team_lines as _tl
                            g = _EP.get_game(p.get("sport"), str(gid), target)
                            if g and g.get("prob_home") is not None:
                                lines = _tl(g["prob_home"], g.get("exp_margin"),
                                            p.get("sport")) or {}
                    except Exception as e:
                        out["model_error"] = str(e)[:100]
                ml = lines.get("moneyline") or {}
                sp = lines.get("spread") or {}
                tot = lines.get("total") or {}
                if ml:
                    out["model"]["moneyline"] = {
                        "home_prob": ml.get("home"), "away_prob": ml.get("away"),
                        "home_fair": ml.get("home_odds"), "away_fair": ml.get("away_odds")}
                if sp:
                    out["model"]["spread"] = {
                        "home_line": sp.get("home_line"), "favorite": sp.get("favorite"),
                        "home_cover": sp.get("home_cover"), "away_cover": sp.get("away_cover")}
                if tot:
                    out["model"]["total"] = {
                        "line": tot.get("line"), "projected": tot.get("proj"),
                        "over": tot.get("over"), "under": tot.get("under")}
                # ---- market side (from the existing cache) ----
                try:
                    import odds_api
                    book = odds_api.get_odds(p.get("sport")) or {}
                    hm = p.get("home_name") or p.get("home")
                    aw = p.get("away_name") or p.get("away")
                    rec = None
                    if hm and aw:
                        rec = book.get(odds_api._norm(str(hm)) + "|" + odds_api._norm(str(aw)))
                    if not rec:
                    ...
                    if rec:
                        ...
                        if isinstance(_sp, (int, float)):
                            _sp = {"home_line": _sp}
                        out["market"] = {"moneyline": rec.get("h2h"),
                                         "spread": _sp, "total": _t,
                                         "books": rec.get("books"),
                                         "home": rec.get("home"),
                                         "away": rec.get("away")}
                        # the play already carries the enriched market price;
                        # use it when the raw h2h block is empty
                        if not rec.get("h2h"):
                            try:
                                _m._enrich_odds(p)
                            except Exception:
                            ...
                            if p.get("market_odds") is not None:
                                out["market"]["pick_price"] = p.get("market_odds")
                                out["market"]["pick"] = p.get("pick")
                except Exception as e:
                    out["market_error"] = str(e)[:100]
                # ---- edges ----
                mk = out.get("market") or {}
                mkml = mk.get("moneyline") or {}
                if ml and mkml:
                    for side, prob in (("home", ml.get("home")), ("away", ml.get("away"))):
                        price = mkml.get(f"{side}_odds") or mkml.get(side)
                        e = _edge(prob, price)
                        if e is not None:
                            out["edges"][f"moneyline_{side}"] = e
                mkt_tot = mk.get("total") or {}
                if tot and mkt_tot:
                    lineval = mkt_tot.get("line") or mkt_tot.get("point")
                    proj = tot.get("proj")
                    if lineval is not None and proj is not None:
                        try:
                            out["edges"]["total_projection_vs_line"] = round(
                                float(proj) - float(lineval), 2)
                        except (TypeError, ValueError):
                            pass
                    for side in ("over", "under"):
                        e = _edge(tot.get(side), mkt_tot.get(f"{side}_odds"))
                        if e is not None:
                            out["edges"][f"total_{side}"] = e
                mkt_sp = mk.get("spread") or {}
                if sp and mkt_sp:
                    mline = mkt_sp.get("home_line") or mkt_sp.get("point")
                    if mline is not None and sp.get("home_line") is not None:
                        try:
                            out["edges"]["spread_model_minus_market"] = round(
                                float(sp["home_line"]) - float(mline), 2)
                        except (TypeError, ValueError):
                            pass
                return out
        return {"found": False, "team": team, "searched_for": needle,
                "note": f"No game found in the next {days} day(s)."}
    except Exception as e:
        import traceback
...
@router.get("/api/ai/diag")
def ai_diag(token: str = "", authorization: str | None = Header(None)):
    """Is the AI write-up path actually working? Reports whether a key is present,
    which model is configured, and the result of one real (tiny) call."""
    ok = bool(_CRON_TOKEN) and (token or "").strip() == _CRON_TOKEN
    if not ok and not _is_admin(authorization):
        return {"error": "forbidden"}
    import os as _os
    key = _os.environ.get("ANTHROPIC_API_KEY", "")
    model = (_os.environ.get("CHAT_MODEL")
             or _os.environ.get("ANTHROPIC_MODEL") or "(not set)")
    out = {
        "key_present": bool(key),
        "key_length": len(key) if key else 0,
        "key_tail": key[-4:] if len(key) > 8 else None,
        "model_configured": model,
    }
    if not key:
        out["verdict"] = "No ANTHROPIC_API_KEY set — write-ups cannot work."
        return out
    try:
        import httpx
        r = httpx.post(
            ...
            json={"model": model if model != "(not set)" else "claude-sonnet-4-5",
                  "max_tokens": 16,
        ...
        if r.status_code == 200:
            body = r.json()
            txt = "".join(b.get("text", "") for b in (body.get("content") or []))
            out["reply"] = txt[:40]
            out["verdict"] = "AI is working."
        else:
            out["error_body"] = (r.text or "")[:300]
            out["verdict"] = {
                401: "Key rejected — rotate/replace ANTHROPIC_API_KEY.",
                400: "Bad request — usually an invalid model name.",
                404: "Model not found — check the model string.",
                429: "Rate limited or out of credit.",
            }.get(r.status_code, "Call failed; see error_body.")
    except Exception as e:
        out["verdict"] = f"Request raised: {str(e)[:160]}"
    return out


@router.get("/api/tennis/audit")
def tennis_audit(token: str = "", authorization: str | None = Header(None)):
    """Where exactly are tennis losses coming from? Segments every settled tennis
    pick by tour, model confidence, price, and edge-vs-market, and reports CLV
    per segment. Diagnosis before adjustment — the loss is almost never uniform.
    """
    ok = bool(_CRON_TOKEN) and (token or "").strip() == _CRON_TOKEN
    if not ok and not _is_admin(authorization):
        return {"error": "forbidden"}
    try:
        ...
        def dec(o):
            o = float(o)
            return (o / 100.0 + 1) if o > 0 else (100.0 / abs(o) + 1)
        ...
        def add(group, label, r, beat):
            b = buckets.setdefault(group, {}).setdefault(
                label, {"n": 0, "w": 0, "units": 0.0, "clv_n": 0, "clv_beat": 0,
                        "priced": 0})
            b["n"] += 1
            if r.correct:
                b["w"] += 1
            if r.taken_odds is not None:
                b["priced"] += 1
                b["units"] += (dec(r.taken_odds) - 1) if r.correct else -1.0
            if beat is not None:
                b["clv_n"] += 1
                b["clv_beat"] += 1 if beat else 0

        with SessionLocal() as db:
            rows = db.execute(
                select(PickResult).where(PickResult.sport == "tennis")).scalars().all()
            for r in rows:
                sub = (r.subcat or "UNTAGGED").upper()
                if sub in ("ITF",):
                    continue                     # excluded from the tracked record
                beat = None
                if r.taken_odds is not None and r.close_odds is not None:
                    # you "beat the close" when your price was better than closing
                    beat = dec(r.taken_odds) > dec(r.close_odds)
                add("by_tour", sub, r, beat)
                if r.prob is not None:
                    p = float(r.prob)
                    lab = ("50-55%" if p < .55 else "55-60%" if p < .60 else
                           "60-70%" if p < .70 else "70-80%" if p < .80 else "80%+")
                    add("by_confidence", lab, r, beat)
                if r.taken_odds is not None:
                    o = int(r.taken_odds)
                    lab = ("heavy fav <-200" if o <= -200 else
                           "fav -200..-120" if o <= -120 else
                           "coinflip -120..+120" if o <= 120 else
                           "dog +120..+250" if o <= 250 else "longshot +250+")
                    add("by_price", lab, r, beat)
                if r.prob is not None and r.taken_odds is not None:
                    e = (float(r.prob) - implied(r.taken_odds)) * 100
                    lab = ("negative" if e < 0 else "0-5%" if e < 5 else
                           "5-10%" if e < 10 else "10-20%" if e < 20 else "20%+")
                    add("by_edge", lab, r, beat)

        out = {}
        for group, labels in buckets.items():
            out[group] = {}
            for lab, b in sorted(labels.items()):
                out[group][lab] = {
                    "picks": b["n"],
                    "win_pct": round(100 * b["w"] / b["n"], 1) if b["n"] else None,
                    "units": round(b["units"], 2),
                    "roi_pct": (round(100 * b["units"] / b["priced"], 1)
                                if b["priced"] else None),
                    "clv_beat_pct": (round(100 * b["clv_beat"] / b["clv_n"], 1)
                                     if b["clv_n"] else None),
                    "clv_sample": b["clv_n"],
                }
        out["_reading"] = (
            "CLV under ~50% means the market moved AGAINST your price — the "
            "strongest evidence a segment's edge isn't real. Look for segments "
            "with many picks, negative units AND low CLV: those are the ones to "
            "stop betting.")
        return out
    except Exception as e:
        import traceback
