...
def build_ladder(day=None):
    """Today's ladder: best-edge picks priced between LADDER_LO and LADDER_HI, one
    per game, ordered by start time so the legs are actually sequenceable."""
    day = day or dt.date.today()
    src = _locked_picks_for(day)
    if not src and day == dt.date.today():
        try:
            import main
            src = main._gather_plays(day)
        except Exception:
            src = []
    # in-band, real edge, dedup by game, best edge first
    seen, cands = set(), []
    for p in src or []:
        mkt = p.get("market_odds")
        edge = p.get("edge_pct")
        gid = p.get("id") or p.get("game_id") or p.get("match")
        if mkt is None or edge is None or not _in_band(mkt):
            continue
        try:
            if float(edge) <= 0:
                continue
        ...
        seen.add(gid)
        cands.append(p)
    cands.sort(key=lambda p: -float(p.get("edge_pct") or 0))
    legs = cands[:_LADDER_RUNGS]
    if not legs:
        return None
    # order the chosen legs by kickoff so they ladder in sequence
    legs.sort(key=lambda p: str(p.get("start") or p.get("time") or ""))
    bankroll = _LADDER_START
    rungs, prob_all = [], 1.0
    for i, p in enumerate(legs, 1):
        pick, pct = _pick_line(p)
        mkt = int(p.get("market_odds"))
        payout = round(bankroll * _dec_odds(mkt), 2)
        try:
            prob_all *= float(p.get("prob") or 0)
        except (TypeError, ValueError):
            pass
        rungs.append({"rung": i, "pick": pick, "odds": mkt,
                      "edge_pct": p.get("edge_pct"),
                      "stake": round(bankroll, 2), "to_return": payout,
                      "sport": p.get("sport")})
        bankroll = payout
    return {"date": day.isoformat(), "start": _LADDER_START,
            "target": round(bankroll, 2), "rungs": rungs,
            "completion_prob_pct": round(prob_all * 100, 3),
            "band": [_LADDER_LO, _LADDER_HI]}


def _ladder_text(l):
    """Discord/X text for the DAILY ladder pick + running challenge state."""
    if not l or not l.get("leg"):
        return ("\U0001FA9C Line Logic Ladder Challenge\nNo qualifying pick today "
                f"(need a model edge priced +100 to -120). The run holds at rung "
                f"{(l or {}).get('rung', 1)} — we don't force a leg.")
    leg, st = l["leg"], l["state"]
    o = f"+{leg['odds']}" if leg["odds"] > 0 else str(leg["odds"])
    lines = [
        f"\U0001FA9C **Line Logic Ladder Challenge \u2014 Day {leg['rung']}/10**",
        f"Attempt #{st['attempt']} \u00b7 bankroll ${leg['stake']:g}",
        "",
        f"\U0001F3AF {leg['pick']} ({o})",
        f"Model edge: +{leg['edge_pct']}%",
        f"${leg['stake']:g} \u2192 ${leg['to_return']:g} if it hits",
    ]
    if st.get("best_rung_ever"):
        lines.append(f"\nBest run so far: reached rung {st['best_rung_ever']}"
                     + (f" \u00b7 {st['completed_runs']} full ladders completed"
                        if st.get("completed_runs") else ""))
    lines.append("\nOne pick a day \u00b7 win rolls forward, loss resets to $10. "
                 "Tracked publicly. Not betting advice.")
    return "\n".join(lines)


def build_ladder(day=None):
    """Today's DAILY ladder pick + running challenge state. One best-edge in-band
    pick; skips the day if none qualifies. Persistent across days (see ladder.py)."""
    day = day or dt.date.today()
    src = _locked_picks_for(day)
    if not src and day == dt.date.today():
        try:
            ...
            src = []
    # _gather_plays doesn't attach market odds/edge — enrich so the ladder can
    # filter on REAL market price (the +100/-120 band) and the model's edge.
    try:
        import main as _m
        for p in (src or []):
    ...
    try:
        ...
        if not leg:
            return {"date": day.isoformat(), "leg": None, "rung": st["current_rung"],
                    "state": st}
        return {"date": day.isoformat(), "state": st,
        ...
        return {"date": day.isoformat(), "leg": None, "error": str(e)[:150]}
