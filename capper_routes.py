    ...
    if look.get("side_is_model_pick"):
        ...
        if mo is not None:
            try:
                ...
                out["track_note"] = f"user took non-model side; inverted {mo} -> {coerced}"
            except Exception as e:
                out["track_would_store"] = None
                out["track_note"] = f"inversion failed: {e}"
        else:
            out["track_would_store"] = None
            out["track_note"] = "no model-side price to invert"
...
async def capper_track(payload: dict):
    ...
    if not is_model_pick and prob is not None:
        try:
            ...
            prob = None
    # The board prices the MODEL's side only. If the user tracked the other
    # side, don't book the model-side price (that game was never offered at that
    # number for this side) — instead INVERT it to the fair price for their
    # side, so the slip shows a real number instead of a blank "\u2014". Flag it
    # as estimated so downstream grading knows it wasn't a booked line.
        ...
        if odds is not None:
            try:
                ...
                if ip is not None and 0 < ip < 1:
                    inv = american_odds(1.0 - ip)   # fair price for the other side
                    # american_odds returns a string like "-140"/"+120"; coerce
