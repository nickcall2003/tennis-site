...
def main():
    ...
    ap.add_argument("--futures", action="store_true",
                    help="also include ITF futures / ITF-level matches (adds ITF player ratings; bigger file)")
    args = ap.parse_args()
    ...
    scope = ("tour + challenger" if not args.no_challengers else "tour only") + (" + ITF futures" if args.futures else "")
