...
def refresh_sp_file(year=None, path=None):
    ...
    This is the piece that makes college football self-maintaining: ratings move
    every week during the season, and until now that file had to be rebuilt by
    hand. Returns a summary; never raises.
    """
    year = year or season_year()
    sp = sp_ratings(year)
    if not sp:
        return {"ok": False, "error": "no SP+ data returned (check CFBD_KEY)",
                "season": year}
    teams, defs = {}, []
    # use the raw rows so we keep the ORIGINAL team names for the keys
    rows = _get(f"sp_{year}", "/ratings/sp", {"year": year})
    for r in (rows or []):
        name = r.get("team")
        if not name:
        ...
        if isinstance(d, (int, float)):
            defs.append(d)
    if not teams:
        return {"ok": False, "error": "parsed 0 teams", "season": year}
    national = {"def": round(sum(defs) / len(defs), 3) if defs else None}
    blob = {"teams": teams, "national": national, "season": year,
