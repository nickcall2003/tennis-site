...
def key_info():
    ...
    raw = os.environ.get("CFBD_KEY") or os.environ.get("CFBD_API_KEY")
    if raw is None:
...
def _norm_team(s):
    """Match ncaaf_provider's normalizer so keys line up exactly."""
    import unicodedata
...
def _sp_rows(year):
    """Raw SP+ rows for a season (cached via _get). [] when none/failed."""
    r = _get(f"sp_{year}", "/ratings/sp", {"year": year})
    return r if isinstance(r, list) else []


def refresh_sp_file(year=None, path=None):
    """Rebuild the SP+ file that ncaaf_provider.predict() reads, straight from the
    live CFBD API, then hot-reload the provider.

    Self-maintaining: ratings move every week in season. Before a new season's SP+
    is published (e.g. 2026 preseason in August, CFBD returns an empty list), we
    fall back to the most recent season that HAS data and self-heal to the new
    season the moment CFBD posts it. Returns a summary; never raises.
    """
    requested = int(year or season_year())
    fallback_years = int(os.environ.get("CFBD_FALLBACK_YEARS", "3"))

    rows, used, tried = [], None, []
    for cand in range(requested, requested - max(1, fallback_years) - 1, -1):
        r = _sp_rows(cand)
        tried.append({"year": cand, "rows": len(r)})
        if r:
            rows, used = r, cand
            break

    if not rows:
        # Distinguish "CFBD rejected us" (auth/tier) from "season just empty".
        h = _health.get(f"sp_{requested}", {})
        if h.get("error"):
            reason = f"CFBD request problem: {h['error']}"
        else:
            reason = ("CFBD returned 0 SP+ rows for every season tried — a valid key "
                      "should have prior-year SP+, so this points to the key's tier "
                      "not including SP+ ratings.")
        return {"ok": False, "error": f"no SP+ data returned. {reason}",
                "season": requested, "tried": tried, "key_info": key_info()}
    ...
    for r in rows:
        ...
        o = off.get("rating") if isinstance(off, dict) else off
        d = dfn.get("rating") if isinstance(dfn, dict) else dfn
        if r.get("rating") is None:
            continue
        teams[_norm_team(name)] = {
            "sp": r.get("rating"), "off": o, "def": d, "name": name,
        }
        if isinstance(d, (int, float)):
            defs.append(d)
    if not teams:
        return {"ok": False, "error": "parsed 0 teams", "season": used}
    national = {"def": round(sum(defs) / len(defs), 3) if defs else None}
    blob = {"teams": teams, "national": national, "season": used,
            "requested_season": requested, "fallback": used != requested,
            "updated": dt.datetime.now(dt.timezone.utc).isoformat()}
    target = path or os.environ.get("NCAAF_SP_PATH") or (
        "/data/ncaaf_sp.json" if os.path.isdir("/data") else "ncaaf_sp.json")
    try:
    ...
    try:
        ...
        pass
    return {"ok": True, "season": used, "requested_season": requested,
            "fallback": used != requested, "teams": len(teams), "path": target,
            "national_def": national["def"], "provider_reloaded": reloaded}


# ----------------------------- FBS enumeration (win totals) -----------------------------
...
def fbs_teams(year=None):
    """Authoritative FBS team list from CFBD (~134 schools). Returns
    [{name, id, abbr, conference}]. This is the reliable league enumerator for the
    win-totals page — ESPN's /teams endpoint mixes divisions and drops teams."""
    year = year or season_year()
    rows = _get(f"fbs_teams_{year}", "/teams/fbs", {"year": year})
    if not isinstance(rows, list) or not rows:
        rows = _get("fbs_teams_noyear", "/teams/fbs")     # some versions ignore year
    if not isinstance(rows, list):
        return []
    out = []
    for r in rows:
        nm = _pick(r, "school", "team", "name")
        if not nm:
            continue
        out.append({"name": nm, "id": _pick(r, "id"),
                    "abbr": _pick(r, "abbreviation", default="") or "",
                    "conference": _pick(r, "conference")})
    return out


def team_schedules(year=None):
    """Every FBS team's full REGULAR-season schedule from CFBD, grouped by school
    name: {name: [{opponent, home, completed, won, date}, ...]}. Everything keys on
    the same school names SP+ uses, so no cross-source ID matching is needed."""
    year = year or season_year()
    rows = _get(f"games_reg_{year}", "/games", {"year": year, "seasonType": "regular"})
    if not isinstance(rows, list) or not rows:
        return {}
    ...
    for g in rows:
        ...
        date = _pick(g, "startDate", "start_date")
        decided = completed and hp is not None and ap is not None
        sched[home].append({"opponent": away, "home": True, "completed": completed,
                             "won": bool(decided and hp > ap), "date": date})
        sched[away].append({"opponent": home, "home": False, "completed": completed,
                             "won": bool(decided and ap > hp), "date": date})
    return dict(sched)
