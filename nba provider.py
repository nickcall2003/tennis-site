...
def predict(home_name, away_name):
    ...
    return {
        ...
        "model": "nba-eff",
        "factors": {
            "home_off": round(h["off"], 1), "away_off": round(a["off"], 1),
            "home_def": round(h["def"], 1), "away_def": round(a["def"], 1),
            "home_net": round(h.get("net", 0.0), 1), "away_net": round(a.get("net", 0.0), 1),
            "pace": round(game_pace, 1), "home_edge": HOME_COURT,
            "exp_margin": round(margin, 1), "avg_total": round(total, 1),
        },
    }
