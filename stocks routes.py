...
def hot_pick():
    """The steadiest climber across the watchlist \u2014 the daily 'hot pick', with a
    real volume-vs-average read for the winner (buying-interest signal)."""
    import stocks_engine as E
    import stocks_data as D
    ...
    if best:
        try:
            best["vol_ratio"] = D.volume_ratio(best["ticker"])
        except Exception:
            best["vol_ratio"] = None
    return best
...
def _alert_text(buys, sells, hot=None):
    ...
    if hot:
        ...
        vol_line = ""
        if vr and vr >= 1.2:
            vol_line = (" Trading on **" + str(round((vr - 1) * 100)) +
                        "% above its usual volume** \u2014 real buying interest.")
        lines += ["\U0001F525 **Hot Pick of the Day: " + hot["ticker"] + "** (" + hot["name"] + ")",
                  ...
                  ", flagged for consistent day-by-day momentum (not a one-day spike)." + vol_line, ""]
    if buys:
