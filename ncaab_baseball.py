...
def get_games(date: dt.date, force_live=False):
    ...
    for ev in data.get("events", []):
        ...
        if a["rank"]:
            prominence += 0.5
        # ESPN sometimes carries a live base/out block on the baseball scoreboard;
        # extract defensively so the diamond lights up when present, else stays hidden.
        _sit = comp.get("situation") or {}
        situation = ({
            "outs": _sit.get("outs"),
            "balls": _sit.get("balls"),
            "strikes": _sit.get("strikes"),
            "on1": bool(_sit.get("onFirst")),
            "on2": bool(_sit.get("onSecond")),
            "on3": bool(_sit.get("onThird")),
            "batter": (((_sit.get("batter") or {}).get("athlete") or {}).get("displayName")
                       or (_sit.get("batter") or {}).get("displayName")),
            "pitcher": (((_sit.get("pitcher") or {}).get("athlete") or {}).get("displayName")
                        or (_sit.get("pitcher") or {}).get("displayName")),
        } if _sit else None)
        games.append({
            ...
            "score": {"home": h["score"], "away": a["score"],
                      "detail": st.get("shortDetail", ""), "situation": situation},
...
def get_boxscore(date, game_id):
    """Live college-baseball box score (reuses the espn_provider parser)."""
    try:
        data = _get(SUMMARY, {"event": str(game_id)})
    except Exception as e:
        print(f"[ncaabb] boxscore fetch failed: {e}")
        return {"teams": []}
    from espn_provider import _parse_espn_boxscore
    return _parse_espn_boxscore(data)
