...
def get_history(tickers, lookback_days=200):
    ...
    ts = _time_series(tickers, "1day", min(5000, lookback_days + 5), pace=2.0)
    if ts:
        _hist_cache["t"] = now
...
# ---- market movers via Alpha Vantage (free, one call, real market-wide) ------
_AV_KEY = os.environ.get("ALPHAVANTAGE_API_KEY", "").strip()
_mv_cache = {"t": None, "data": None}


def alpha_movers():
    """Real market-wide top gainers/losers from Alpha Vantage TOP_GAINERS_LOSERS
    (one free call). Cached ~10 min (free tier is 25 calls/day). None if no key."""
    if not _AV_KEY:
        return None
    now = dt.datetime.utcnow()
    if _mv_cache["data"] and _mv_cache["t"] and (now - _mv_cache["t"]).total_seconds() < 600:
        return _mv_cache["data"]
    try:
        import httpx
        r = httpx.get("https://www.alphavantage.co/query",
                      params={"function": "TOP_GAINERS_LOSERS", "apikey": _AV_KEY}, timeout=12.0)
        d = r.json()
    except Exception:
        return _mv_cache["data"]
    ...
    def parse(key):
        rows = []
        for v in (d.get(key) or [])[:10]:
            try:
                pct = float(str(v.get("change_percentage", "")).replace("%", ""))
                rows.append({"ticker": v["ticker"], "name": v["ticker"],
                             "price": round(float(v["price"]), 2), "change_pct": round(pct, 2),
                             "series": []})
            except Exception:
                pass
        return rows
    g, l = parse("top_gainers"), parse("top_losers")
    if not g and not l:
        return None
    out = {"gainers": g, "losers": l}
    _mv_cache["t"] = now
    _mv_cache["data"] = out
    return out
