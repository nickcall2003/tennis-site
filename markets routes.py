...
def market_game(sport: str, ref: str, best_of: int = 3):
    ...
    if prob_home is None:
        # board plays carry the PICK's prob + pick side; reconstruct home prob
        pk = (play.get("pick") or "").lower()
        pr = play.get("prob")
        if pr is not None and home_nm and pk:
            prob_home = pr if home_nm.lower().find(pk.split()[-1]) >= 0 else 1 - pr
        else:
            prob_home = pr
    ctx = play.get("ctx") or {}
