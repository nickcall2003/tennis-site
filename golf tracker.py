...
def settle(tour="pga", board=None, stale_days=6):
    ...
    board_event = _evnorm(((board or {}).get("event") or {}).get("name"))
    ...
    with SessionLocal() as db:
        ...
        for mp in pend:
            try:
                ...
                # Only snapshot from a board that is the SAME EVENT as the pick —
                # otherwise a player who is in this week's field too would have
                # the wrong tournament's score pulled into an old matchup.
                mp_event = _evnorm(mp.event)
                event_ok = bool(board_event) and bool(mp_event) and (
                    board_event[:12] == mp_event[:12]
                    or board_event in mp_event or mp_event in board_event)
