...
def _favicon():
    import os as _os
    if _os.path.exists("favicon.ico"):
...
def _favicon_png():
    import os as _os
    for p in ("favicon-32x32.png", "favicon-16x16.png"):
        if _os.path.exists(p):
            return FileResponse(p, media_type="image/png",
                                headers={"Cache-Control": "no-cache, must-revalidate"})
...
def _apple_icon():
    import os as _os
    for p in ("apple-touch-icon.png", "icon-180.png"):
        if _os.path.exists(p):
            return FileResponse(p, media_type="image/png",
                                headers={"Cache-Control": "no-cache, must-revalidate"})
    return Response(content=_ICON_180, media_type="image/png",
...
def _service_worker():
    ...
    """
    js = """
const CACHE = "ll-shell-v14";   // bumped: evicts the pre-redesign shell so the new home/palette loads
